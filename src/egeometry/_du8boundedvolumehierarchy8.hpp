...
#ifndef EGEOMETRY_DU8BOUNDEDVOLUMEHIERARCHY8_HPP
#define EGEOMETRY_DU8BOUNDEDVOLUMEHIERARCHY8_HPP
...
#include "_du8boundedvolumehierarchy8type.hpp"
#include "_modulestate.hpp"

static const uint8_t DU8BoundedVolumeHierarchy8_LEAF_MASK (uint8_t(1) << (sizeof(uint8_t) * 8 - 1));

struct DU8BoundedVolumeHierarchy8Items
{
    size_t count;
    double *min_x;
    double *min_y;
...
static DU8BoundedVolumeHierarchy8Items
DU8BoundedVolumeHierarchy8_alloc_items_from_py_sequence_bounding_box_3d(
    PyObject *py_items
)
{
    ...
    DU8BoundedVolumeHierarchy8Items items;
    items.count = 0;
    items.min_x = 0;
...
static void
DU8BoundedVolumeHierarchy8_free_items(DU8BoundedVolumeHierarchy8Items items)
{
    if (items.min_x == 0)
    {
        free(items.min_x);
...
static void
DU8BoundedVolumeHierarchy8_alloc_nodes(
    DU8BoundedVolumeHierarchy8Items items,
...
)
{
    ...
    if (items.count == 0)
    {
        *nodes = (DU8BoundedVolumeHierarchy8Node *)malloc(1);
        if (!nodes)
        {
            PyErr_NoMemory();
            return;
    ...
    size_t max_item_count = (size_t)(uint8_t)(~DU8BoundedVolumeHierarchy8_LEAF_MASK) + 1;
    ...
    double *center_y = 0;
    double *center_z = 0;
    DU8BoundedVolumeHierarchy8Node *node_array = 0;
    ...
    node_array = (DU8BoundedVolumeHierarchy8Node *)malloc(sizeof(DU8BoundedVolumeHierarchy8Node) * node_array_count);
    if (!node_array){ PyErr_NoMemory(); goto error; }
    memset(node_array, 0, sizeof(DU8BoundedVolumeHierarchy8Node) * node_array_count);
...

    {
        ...
        auto fill_child = [&](size_t node_idx, int c, size_t child_start, size_t child_end) -> bool
        {
            ...
            else if (child_count_items == 1)
            {
                ...
                node_array[node_idx].child[c] = item_idx | DU8BoundedVolumeHierarchy8_LEAF_MASK;
                return true;
            }
            else
            {
        ...
        {
            DU8BoundedVolumeHierarchy8Node *shrunk = (DU8BoundedVolumeHierarchy8Node *)realloc(node_array, sizeof(DU8BoundedVolumeHierarchy8Node) * next_node);
...
static void
DU8BoundedVolumeHierarchy8_free_nodes(DU8BoundedVolumeHierarchy8Node *nodes)
{
    if (nodes)
    {
        free(nodes);
...
static PyObject *
DU8BoundedVolumeHierarchy8__new__(PyTypeObject *cls, PyObject *args, PyObject *kwds)
{
    DU8BoundedVolumeHierarchy8 *self = 0;
    DU8BoundedVolumeHierarchy8Node *nodes = 0;

    static const char *kwp[] = {"items", 0};
    PyObject *py_items = 0;
    if (!PyArg_ParseTupleAndKeywords(args, kwds, "O", (char **)kwp, &py_items)){ return 0; }

    DU8BoundedVolumeHierarchy8Items items = DU8BoundedVolumeHierarchy8_alloc_items_from_py_sequence_bounding_box_3d(py_items);
    if (items.min_x == 0){ goto error; }

    size_t node_count;
    DU8BoundedVolumeHierarchy8_alloc_nodes(items, &nodes, &node_count);
    if (!nodes && items.count > 0){ goto error; }

    DU8BoundedVolumeHierarchy8_free_items(items);

    self = (DU8BoundedVolumeHierarchy8*)cls->tp_alloc(cls, 0);
    if (!self)
    {
        goto error;
    }
...
error:
    DU8BoundedVolumeHierarchy8_free_nodes(nodes);
    DU8BoundedVolumeHierarchy8_free_items(items);
    return 0;
}

static void
DU8BoundedVolumeHierarchy8__dealloc__(DU8BoundedVolumeHierarchy8 *self)
{
    if (self->weakreflist)
    {
...
static PyObject *
DU8BoundedVolumeHierarchy8__repr__(DU8BoundedVolumeHierarchy8 *self)
{
...
static PyObject *
DU8BoundedVolumeHierarchy8_nodes(DU8BoundedVolumeHierarchy8 *self, void *closure)
{
    PyObject *bbox_cls = 0;
    PyObject *result = 0;
    ...
    {
        DU8BoundedVolumeHierarchy8Node *node = &self->nodes[n];
        ...
        for (int c = 0; c < 8; c++)
        {
            ...
            if (child_val & DU8BoundedVolumeHierarchy8_LEAF_MASK)
            {
                // leaf node - just emit the item index
                uint8_t item_idx = child_val & ~DU8BoundedVolumeHierarchy8_LEAF_MASK;
...
struct DU8BoundedVolumeHierarchy8RaycastIterator
{
    PyObject_HEAD
    DU8BoundedVolumeHierarchy8 *bvh;
    double eye[3];
    double inv_dir[3];
...
static void
DU8BoundedVolumeHierarchy8RaycastIterator__dealloc__(DU8BoundedVolumeHierarchy8RaycastIterator *self)
{
    Py_XDECREF(self->bvh);
    if (self->stack)
    {
...
static inline void
DU8BoundedVolumeHierarchy8_ray_test_node(
    const double *eye,
    const double *inv_dir,
    const DU8BoundedVolumeHierarchy8Node *node,
    uint8_t *hits,
    int *hit_count
)
{
...
static PyObject *
DU8BoundedVolumeHierarchy8RaycastIterator__next__(DU8BoundedVolumeHierarchy8RaycastIterator *self)
{
    DU8BoundedVolumeHierarchy8 *bvh = self->bvh;
    ...
    while (true)
    {
        ...
        DU8BoundedVolumeHierarchy8Node *node = &bvh->nodes[node_idx];

        // test all children of this node
        DU8BoundedVolumeHierarchy8_ray_test_node(self->eye, self->inv_dir, node, self->hits, &self->hit_count);
        self->hit_index = 0;
    }
}

static PyObject *
DU8BoundedVolumeHierarchy8RaycastIterator__iter__(DU8BoundedVolumeHierarchy8RaycastIterator *self)
{
    Py_INCREF(self);
    return (PyObject *)self;
}

static PyType_Slot DU8BoundedVolumeHierarchy8RaycastIterator_PyType_Slots[] = {
    {Py_tp_dealloc, (void *)DU8BoundedVolumeHierarchy8RaycastIterator__dealloc__},
    {Py_tp_iter, (void *)DU8BoundedVolumeHierarchy8RaycastIterator__iter__},
    {Py_tp_iternext, (void *)DU8BoundedVolumeHierarchy8RaycastIterator__next__},
    {0, 0},
};

static PyType_Spec DU8BoundedVolumeHierarchy8RaycastIterator_PyTypeSpec = {
    "egeometry.DU8BoundedVolumeHierarchy8RaycastIterator",
    sizeof(DU8BoundedVolumeHierarchy8RaycastIterator),
    0,
    Py_TPFLAGS_DEFAULT,
    DU8BoundedVolumeHierarchy8RaycastIterator_PyType_Slots
};

static PyTypeObject *DU8BoundedVolumeHierarchy8RaycastIterator_PyTypeObject = 0;

static PyObject *
DU8BoundedVolumeHierarchy8_raycast(DU8BoundedVolumeHierarchy8 *self, PyObject *const *args, Py_ssize_t nargs)
{
    if (nargs != 2)
    {
    ...
    // create iterator
    if (!DU8BoundedVolumeHierarchy8RaycastIterator_PyTypeObject)
    {
        DU8BoundedVolumeHierarchy8RaycastIterator_PyTypeObject = (PyTypeObject *)PyType_FromSpec(
            &DU8BoundedVolumeHierarchy8RaycastIterator_PyTypeSpec);
        if (!DU8BoundedVolumeHierarchy8RaycastIterator_PyTypeObject){ return 0; }
    }

    DU8BoundedVolumeHierarchy8RaycastIterator *iter = PyObject_New(
        DU8BoundedVolumeHierarchy8RaycastIterator, DU8BoundedVolumeHierarchy8RaycastIterator_PyTypeObject);
    if (!iter){ return 0; }
    ...
    {
        DU8BoundedVolumeHierarchy8_ray_test_node(iter->eye, iter->inv_dir, &self->nodes[0], iter->hits, &iter->hit_count);
    }
    else
    {
        iter->hit_count = 0;
...
static PyMethodDef DU8BoundedVolumeHierarchy8_PyMethodDef[] = {
    {"raycast", (PyCFunction)DU8BoundedVolumeHierarchy8_raycast, METH_FASTCALL, 0},
    {0, 0, 0, 0}
};

static PyMemberDef DU8BoundedVolumeHierarchy8_PyMemberDef[] = {
    {"__weaklistoffset__", T_PYSSIZET, offsetof(DU8BoundedVolumeHierarchy8, weakreflist), READONLY},
    {0, 0, 0, 0, 0},
};

static PyGetSetDef DU8BoundedVolumeHierarchy8_PyGetSetDef[] = {
    {"nodes", (getter)DU8BoundedVolumeHierarchy8_nodes, 0, 0, 0},
    {0, 0, 0, 0, 0}
};

static PyType_Slot DU8BoundedVolumeHierarchy8_PyType_Slots [] = {
    {Py_tp_new, (void*)DU8BoundedVolumeHierarchy8__new__},
    {Py_tp_dealloc, (void*)DU8BoundedVolumeHierarchy8__dealloc__},
    {Py_tp_repr, (void*)DU8BoundedVolumeHierarchy8__repr__},
    {Py_tp_members, (void*)DU8BoundedVolumeHierarchy8_PyMemberDef},
    {Py_tp_getset, (void*)DU8BoundedVolumeHierarchy8_PyGetSetDef},
    {Py_tp_methods, (void*)DU8BoundedVolumeHierarchy8_PyMethodDef},
    {0, 0},
};

static PyType_Spec DU8BoundedVolumeHierarchy8_PyTypeSpec = {
    "egeometry.DU8BoundedVolumeHierarchy8",
    sizeof(DU8BoundedVolumeHierarchy8),
    0,
    Py_TPFLAGS_DEFAULT,
    DU8BoundedVolumeHierarchy8_PyType_Slots
};

static PyTypeObject *
define_DU8BoundedVolumeHierarchy8_type(PyObject *module)
{
    PyTypeObject *type = (PyTypeObject *)PyType_FromModuleAndSpec(
        module,
        &DU8BoundedVolumeHierarchy8_PyTypeSpec,
        0
    );
    if (!type){ return 0; }
    // Note:
    ...
    if (PyModule_AddObject(module, "DU8BoundedVolumeHierarchy8", (PyObject *)type) < 0)
    {
        Py_DECREF(type);
        return 0;
    }

...
#ifndef EGEOMETRY_FU16BOUNDEDVOLUMEHIERARCHY4_HPP
#define EGEOMETRY_FU16BOUNDEDVOLUMEHIERARCHY4_HPP
...
#include "_fu16boundedvolumehierarchy4type.hpp"
#include "_modulestate.hpp"

static const uint16_t FU16BoundedVolumeHierarchy4_LEAF_MASK (uint16_t(1) << (sizeof(uint16_t) * 8 - 1));

struct FU16BoundedVolumeHierarchy4Items
{
    size_t count;
    float *min_x;
    float *min_y;
...
static FU16BoundedVolumeHierarchy4Items
FU16BoundedVolumeHierarchy4_alloc_items_from_py_sequence_bounding_box_3d(
    PyObject *py_items
)
{
    ...
    FU16BoundedVolumeHierarchy4Items items;
    items.count = 0;
    items.min_x = 0;
    ...
    {
        item_data = (char *)malloc(sizeof(float) * 6 * items.count);
    }
    if (!item_data){ PyErr_NoMemory(); goto error; }

    items.min_x = (float*)(item_data + (size_t)(sizeof(float) * items.count * 0));
    items.min_y = (float*)(item_data + (size_t)(sizeof(float) * items.count * 1));
    items.min_z = (float*)(item_data + (size_t)(sizeof(float) * items.count * 2));
    items.max_x = (float*)(item_data + (size_t)(sizeof(float) * items.count * 3));
    items.max_y = (float*)(item_data + (size_t)(sizeof(float) * items.count * 4));
    items.max_z = (float*)(item_data + (size_t)(sizeof(float) * items.count * 5));
...
static void
FU16BoundedVolumeHierarchy4_free_items(FU16BoundedVolumeHierarchy4Items items)
{
    if (items.min_x == 0)
    {
        free(items.min_x);
...
static void
FU16BoundedVolumeHierarchy4_alloc_nodes(
    FU16BoundedVolumeHierarchy4Items items,
    FU16BoundedVolumeHierarchy4Node **nodes,
    size_t *node_count
)
{
    ...
    assert(node_array_count <= (size_t)std::numeric_limits<uint16_t>::max() + 1);

    if (items.count == 0)
    {
        *nodes = (FU16BoundedVolumeHierarchy4Node *)malloc(1);
        if (!nodes)
        {
            PyErr_NoMemory();
            return;
    ...
    // check that item_count can be addressed by uint16_t accounting for LEAF_MASK
    // LEAF_MASK uses the highest bit, so max item count is ~LEAF_MASK + 1 (indices 0 to ~LEAF_MASK)
    // cast to uint16_t to avoid integer promotion issues with ~ operator
    size_t max_item_count = (size_t)(uint16_t)(~FU16BoundedVolumeHierarchy4_LEAF_MASK) + 1;
    ...
    uint16_t *item_indices = 0;
    float *center_x = 0;
    float *center_y = 0;
    float *center_z = 0;
    FU16BoundedVolumeHierarchy4Node *node_array = 0;
    ...
    item_indices = (uint16_t *)malloc(sizeof(uint16_t) * items.count);
    if (!item_indices){ PyErr_NoMemory(); goto error; }
    for (size_t i = 0; i < items.count; i++)
    {
        item_indices[i] = (uint16_t)i;
    }
    ...
    node_array = (FU16BoundedVolumeHierarchy4Node *)malloc(sizeof(FU16BoundedVolumeHierarchy4Node) * node_array_count);
    if (!node_array){ PyErr_NoMemory(); goto error; }
    memset(node_array, 0, sizeof(FU16BoundedVolumeHierarchy4Node) * node_array_count);
...

    {
        ...
        auto partition_items = [&](size_t start, size_t end, size_t *partition_points)
        {
            ...
            while (part_stack_size > 0)
            {
                ...
                if (pt.level >= levels_needed || pt.end - pt.start <= 1)
                {
                    ...
                    int group_size = 4 / groups_at_level;
                ...
                while (left < right)
                {
                    ...
                    if (left < right)
                    {
                        uint16_t tmp = item_indices[left];
        ...
        auto fill_child = [&](size_t node_idx, int c, size_t child_start, size_t child_end) -> bool
        {
            ...
            {
                uint16_t item_idx = item_indices[child_start];
                ...
                node_array[node_idx].child[c] = item_idx | FU16BoundedVolumeHierarchy4_LEAF_MASK;
                return true;
            }
            else
            {
        ...
        while (stack_size > 0)
        {
            ...
            while (c < 4)
            {
                ...
                if (!fill_child(node_idx, c, child_start, child_end))
                {
                    ...
                    node_array[node_idx].child[c] = (uint16_t)child_node;
        ...
        {
            FU16BoundedVolumeHierarchy4Node *shrunk = (FU16BoundedVolumeHierarchy4Node *)realloc(node_array, sizeof(FU16BoundedVolumeHierarchy4Node) * next_node);
...
static void
FU16BoundedVolumeHierarchy4_free_nodes(FU16BoundedVolumeHierarchy4Node *nodes)
{
    if (nodes)
    {
        free(nodes);
...
static PyObject *
FU16BoundedVolumeHierarchy4__new__(PyTypeObject *cls, PyObject *args, PyObject *kwds)
{
    FU16BoundedVolumeHierarchy4 *self = 0;
    FU16BoundedVolumeHierarchy4Node *nodes = 0;

    static const char *kwp[] = {"items", 0};
    PyObject *py_items = 0;
    if (!PyArg_ParseTupleAndKeywords(args, kwds, "O", (char **)kwp, &py_items)){ return 0; }

    FU16BoundedVolumeHierarchy4Items items = FU16BoundedVolumeHierarchy4_alloc_items_from_py_sequence_bounding_box_3d(py_items);
    if (items.min_x == 0){ goto error; }

    size_t node_count;
    FU16BoundedVolumeHierarchy4_alloc_nodes(items, &nodes, &node_count);
    if (!nodes && items.count > 0){ goto error; }

    FU16BoundedVolumeHierarchy4_free_items(items);

    self = (FU16BoundedVolumeHierarchy4*)cls->tp_alloc(cls, 0);
    if (!self)
    {
        goto error;
    }
...
error:
    FU16BoundedVolumeHierarchy4_free_nodes(nodes);
    FU16BoundedVolumeHierarchy4_free_items(items);
    return 0;
}

static void
FU16BoundedVolumeHierarchy4__dealloc__(FU16BoundedVolumeHierarchy4 *self)
{
    if (self->weakreflist)
    {
...
static PyObject *
FU16BoundedVolumeHierarchy4__repr__(FU16BoundedVolumeHierarchy4 *self)
{
...
static PyObject *
FU16BoundedVolumeHierarchy4_nodes(FU16BoundedVolumeHierarchy4 *self, void *closure)
{
    PyObject *bbox_cls = 0;
    PyObject *result = 0;
    ...
    {
        FU16BoundedVolumeHierarchy4Node *node = &self->nodes[n];
        ...
        for (int c = 0; c < 4; c++)
        {
            ...
            uint16_t child_val = node->child[c];

            if (child_val & FU16BoundedVolumeHierarchy4_LEAF_MASK)
            {
                // leaf node - just emit the item index
                uint16_t item_idx = child_val & ~FU16BoundedVolumeHierarchy4_LEAF_MASK;
...
struct FU16BoundedVolumeHierarchy4RaycastIterator
{
    PyObject_HEAD
    FU16BoundedVolumeHierarchy4 *bvh;
    float eye[3];
    float inv_dir[3];
...
static void
FU16BoundedVolumeHierarchy4RaycastIterator__dealloc__(FU16BoundedVolumeHierarchy4RaycastIterator *self)
{
    Py_XDECREF(self->bvh);
    if (self->stack)
    {
...
static inline void
FU16BoundedVolumeHierarchy4_ray_test_node(
    const float *eye,
    const float *inv_dir,
    const FU16BoundedVolumeHierarchy4Node *node,
    uint16_t *hits,
    int *hit_count
)
{
    // test all 4 children simultaneously
...
static PyObject *
FU16BoundedVolumeHierarchy4RaycastIterator__next__(FU16BoundedVolumeHierarchy4RaycastIterator *self)
{
    FU16BoundedVolumeHierarchy4 *bvh = self->bvh;
    ...
    while (true)
    {
        ...
        {
            uint16_t child_val = self->hits[self->hit_index++];
            ...
            else
            {
                ...
                if (self->stack_size >= self->stack_capacity)
                {
                    ...
                    uint16_t *new_stack = (uint16_t *)realloc(
                        self->stack, sizeof(uint16_t) * new_capacity);
                    if (!new_stack)
                    {
                        PyErr_NoMemory();
                        return 0;
        ...
        uint16_t node_idx = self->stack[--self->stack_size];
        FU16BoundedVolumeHierarchy4Node *node = &bvh->nodes[node_idx];

        // test all children of this node
        FU16BoundedVolumeHierarchy4_ray_test_node(self->eye, self->inv_dir, node, self->hits, &self->hit_count);
        self->hit_index = 0;
    }
}

static PyObject *
FU16BoundedVolumeHierarchy4RaycastIterator__iter__(FU16BoundedVolumeHierarchy4RaycastIterator *self)
{
    Py_INCREF(self);
    return (PyObject *)self;
}

static PyType_Slot FU16BoundedVolumeHierarchy4RaycastIterator_PyType_Slots[] = {
    {Py_tp_dealloc, (void *)FU16BoundedVolumeHierarchy4RaycastIterator__dealloc__},
    {Py_tp_iter, (void *)FU16BoundedVolumeHierarchy4RaycastIterator__iter__},
    {Py_tp_iternext, (void *)FU16BoundedVolumeHierarchy4RaycastIterator__next__},
    {0, 0},
};

static PyType_Spec FU16BoundedVolumeHierarchy4RaycastIterator_PyTypeSpec = {
    "egeometry.FU16BoundedVolumeHierarchy4RaycastIterator",
    sizeof(FU16BoundedVolumeHierarchy4RaycastIterator),
    0,
    Py_TPFLAGS_DEFAULT,
    FU16BoundedVolumeHierarchy4RaycastIterator_PyType_Slots
};

static PyTypeObject *FU16BoundedVolumeHierarchy4RaycastIterator_PyTypeObject = 0;

static PyObject *
FU16BoundedVolumeHierarchy4_raycast(FU16BoundedVolumeHierarchy4 *self, PyObject *const *args, Py_ssize_t nargs)
{
    if (nargs != 2)
    {
    ...
    // create iterator
    if (!FU16BoundedVolumeHierarchy4RaycastIterator_PyTypeObject)
    {
        FU16BoundedVolumeHierarchy4RaycastIterator_PyTypeObject = (PyTypeObject *)PyType_FromSpec(
            &FU16BoundedVolumeHierarchy4RaycastIterator_PyTypeSpec);
        if (!FU16BoundedVolumeHierarchy4RaycastIterator_PyTypeObject){ return 0; }
    }

    FU16BoundedVolumeHierarchy4RaycastIterator *iter = PyObject_New(
        FU16BoundedVolumeHierarchy4RaycastIterator, FU16BoundedVolumeHierarchy4RaycastIterator_PyTypeObject);
    if (!iter){ return 0; }
    ...
    iter->stack = (uint16_t *)malloc(sizeof(uint16_t) * iter->stack_capacity);
    if (!iter->stack)
    {
        Py_DECREF(iter);
        PyErr_NoMemory();
    ...
    {
        FU16BoundedVolumeHierarchy4_ray_test_node(iter->eye, iter->inv_dir, &self->nodes[0], iter->hits, &iter->hit_count);
    }
    else
    {
        iter->hit_count = 0;
...
static PyMethodDef FU16BoundedVolumeHierarchy4_PyMethodDef[] = {
    {"raycast", (PyCFunction)FU16BoundedVolumeHierarchy4_raycast, METH_FASTCALL, 0},
    {0, 0, 0, 0}
};

static PyMemberDef FU16BoundedVolumeHierarchy4_PyMemberDef[] = {
    {"__weaklistoffset__", T_PYSSIZET, offsetof(FU16BoundedVolumeHierarchy4, weakreflist), READONLY},
    {0, 0, 0, 0, 0},
};

static PyGetSetDef FU16BoundedVolumeHierarchy4_PyGetSetDef[] = {
    {"nodes", (getter)FU16BoundedVolumeHierarchy4_nodes, 0, 0, 0},
    {0, 0, 0, 0, 0}
};

static PyType_Slot FU16BoundedVolumeHierarchy4_PyType_Slots [] = {
    {Py_tp_new, (void*)FU16BoundedVolumeHierarchy4__new__},
    {Py_tp_dealloc, (void*)FU16BoundedVolumeHierarchy4__dealloc__},
    {Py_tp_repr, (void*)FU16BoundedVolumeHierarchy4__repr__},
    {Py_tp_members, (void*)FU16BoundedVolumeHierarchy4_PyMemberDef},
    {Py_tp_getset, (void*)FU16BoundedVolumeHierarchy4_PyGetSetDef},
    {Py_tp_methods, (void*)FU16BoundedVolumeHierarchy4_PyMethodDef},
    {0, 0},
};

static PyType_Spec FU16BoundedVolumeHierarchy4_PyTypeSpec = {
    "egeometry.FU16BoundedVolumeHierarchy4",
    sizeof(FU16BoundedVolumeHierarchy4),
    0,
    Py_TPFLAGS_DEFAULT,
    FU16BoundedVolumeHierarchy4_PyType_Slots
};

static PyTypeObject *
define_FU16BoundedVolumeHierarchy4_type(PyObject *module)
{
    PyTypeObject *type = (PyTypeObject *)PyType_FromModuleAndSpec(
        module,
        &FU16BoundedVolumeHierarchy4_PyTypeSpec,
        0
    );
    if (!type){ return 0; }
    // Note:
    ...
    if (PyModule_AddObject(module, "FU16BoundedVolumeHierarchy4", (PyObject *)type) < 0)
    {
        Py_DECREF(type);
        return 0;
    }

...
#ifndef EGEOMETRY_FUBOUNDEDVOLUMEHIERARCHY4_HPP
#define EGEOMETRY_FUBOUNDEDVOLUMEHIERARCHY4_HPP
...
#include "_fuboundedvolumehierarchy4type.hpp"
#include "_modulestate.hpp"

static const unsigned int FUBoundedVolumeHierarchy4_LEAF_MASK (unsigned int(1) << (sizeof(unsigned int) * 8 - 1));

struct FUBoundedVolumeHierarchy4Items
{
    size_t count;
    float *min_x;
    float *min_y;
...
static FUBoundedVolumeHierarchy4Items
FUBoundedVolumeHierarchy4_alloc_items_from_py_sequence_bounding_box_3d(
    PyObject *py_items
)
{
    char *item_data = 0;
    ...
    FUBoundedVolumeHierarchy4Items items;
    items.count = 0;
    items.min_x = 0;
...
static void
FUBoundedVolumeHierarchy4_free_items(FUBoundedVolumeHierarchy4Items items)
{
    if (items.min_x == 0)
    {
        free(items.min_x);
...
static void
FUBoundedVolumeHierarchy4_alloc_nodes(
    FUBoundedVolumeHierarchy4Items items,
    FUBoundedVolumeHierarchy4Node **nodes,
    size_t *node_count
)
{
    ...
    if (items.count == 0)
    {
        *nodes = (FUBoundedVolumeHierarchy4Node *)malloc(1);
        if (!nodes)
        {
            PyErr_NoMemory();
            return;
    ...
    size_t max_item_count = (size_t)(unsigned int)(~FUBoundedVolumeHierarchy4_LEAF_MASK) + 1;
    ...
    float *center_y = 0;
    float *center_z = 0;
    FUBoundedVolumeHierarchy4Node *node_array = 0;
    ...
    node_array = (FUBoundedVolumeHierarchy4Node *)malloc(sizeof(FUBoundedVolumeHierarchy4Node) * node_array_count);
    if (!node_array){ PyErr_NoMemory(); goto error; }
    memset(node_array, 0, sizeof(FUBoundedVolumeHierarchy4Node) * node_array_count);
...

    {
        ...
        auto fill_child = [&](size_t node_idx, int c, size_t child_start, size_t child_end) -> bool
        {
            ...
            else if (child_count_items == 1)
            {
                ...
                node_array[node_idx].child[c] = item_idx | FUBoundedVolumeHierarchy4_LEAF_MASK;
                return true;
            }
            else
            {
        ...
        {
            FUBoundedVolumeHierarchy4Node *shrunk = (FUBoundedVolumeHierarchy4Node *)realloc(node_array, sizeof(FUBoundedVolumeHierarchy4Node) * next_node);
...
static void
FUBoundedVolumeHierarchy4_free_nodes(FUBoundedVolumeHierarchy4Node *nodes)
{
    if (nodes)
    {
        free(nodes);
...
static PyObject *
FUBoundedVolumeHierarchy4__new__(PyTypeObject *cls, PyObject *args, PyObject *kwds)
{
    FUBoundedVolumeHierarchy4 *self = 0;
    FUBoundedVolumeHierarchy4Node *nodes = 0;

    static const char *kwp[] = {"items", 0};
    PyObject *py_items = 0;
    if (!PyArg_ParseTupleAndKeywords(args, kwds, "O", (char **)kwp, &py_items)){ return 0; }

    FUBoundedVolumeHierarchy4Items items = FUBoundedVolumeHierarchy4_alloc_items_from_py_sequence_bounding_box_3d(py_items);
    if (items.min_x == 0){ goto error; }

    size_t node_count;
    FUBoundedVolumeHierarchy4_alloc_nodes(items, &nodes, &node_count);
    if (!nodes && items.count > 0){ goto error; }

    FUBoundedVolumeHierarchy4_free_items(items);

    self = (FUBoundedVolumeHierarchy4*)cls->tp_alloc(cls, 0);
    if (!self)
    {
        goto error;
    }
...
error:
    FUBoundedVolumeHierarchy4_free_nodes(nodes);
    FUBoundedVolumeHierarchy4_free_items(items);
    return 0;
}

static void
FUBoundedVolumeHierarchy4__dealloc__(FUBoundedVolumeHierarchy4 *self)
{
    if (self->weakreflist)
    {
...
static PyObject *
FUBoundedVolumeHierarchy4__repr__(FUBoundedVolumeHierarchy4 *self)
{
...
static PyObject *
FUBoundedVolumeHierarchy4_nodes(FUBoundedVolumeHierarchy4 *self, void *closure)
{
    PyObject *bbox_cls = 0;
    PyObject *result = 0;
    ...
    {
        FUBoundedVolumeHierarchy4Node *node = &self->nodes[n];
        ...
        for (int c = 0; c < 4; c++)
        {
            ...
            if (child_val & FUBoundedVolumeHierarchy4_LEAF_MASK)
            {
                // leaf node - just emit the item index
                unsigned int item_idx = child_val & ~FUBoundedVolumeHierarchy4_LEAF_MASK;
...
struct FUBoundedVolumeHierarchy4RaycastIterator
{
    PyObject_HEAD
    FUBoundedVolumeHierarchy4 *bvh;
    float eye[3];
    float inv_dir[3];
...
static void
FUBoundedVolumeHierarchy4RaycastIterator__dealloc__(FUBoundedVolumeHierarchy4RaycastIterator *self)
{
    Py_XDECREF(self->bvh);
    if (self->stack)
    {
...
static inline void
FUBoundedVolumeHierarchy4_ray_test_node(
    const float *eye,
    const float *inv_dir,
    const FUBoundedVolumeHierarchy4Node *node,
    unsigned int *hits,
    int *hit_count
)
{
...
static PyObject *
FUBoundedVolumeHierarchy4RaycastIterator__next__(FUBoundedVolumeHierarchy4RaycastIterator *self)
{
    FUBoundedVolumeHierarchy4 *bvh = self->bvh;
    ...
    while (true)
    {
        ...
        FUBoundedVolumeHierarchy4Node *node = &bvh->nodes[node_idx];

        // test all children of this node
        FUBoundedVolumeHierarchy4_ray_test_node(self->eye, self->inv_dir, node, self->hits, &self->hit_count);
        self->hit_index = 0;
    }
}

static PyObject *
FUBoundedVolumeHierarchy4RaycastIterator__iter__(FUBoundedVolumeHierarchy4RaycastIterator *self)
{
    Py_INCREF(self);
    return (PyObject *)self;
}

static PyType_Slot FUBoundedVolumeHierarchy4RaycastIterator_PyType_Slots[] = {
    {Py_tp_dealloc, (void *)FUBoundedVolumeHierarchy4RaycastIterator__dealloc__},
    {Py_tp_iter, (void *)FUBoundedVolumeHierarchy4RaycastIterator__iter__},
    {Py_tp_iternext, (void *)FUBoundedVolumeHierarchy4RaycastIterator__next__},
    {0, 0},
};

static PyType_Spec FUBoundedVolumeHierarchy4RaycastIterator_PyTypeSpec = {
    "egeometry.FUBoundedVolumeHierarchy4RaycastIterator",
    sizeof(FUBoundedVolumeHierarchy4RaycastIterator),
    0,
    Py_TPFLAGS_DEFAULT,
    FUBoundedVolumeHierarchy4RaycastIterator_PyType_Slots
};

static PyTypeObject *FUBoundedVolumeHierarchy4RaycastIterator_PyTypeObject = 0;

static PyObject *
FUBoundedVolumeHierarchy4_raycast(FUBoundedVolumeHierarchy4 *self, PyObject *const *args, Py_ssize_t nargs)
{
    if (nargs != 2)
    {
    ...
    // create iterator
    if (!FUBoundedVolumeHierarchy4RaycastIterator_PyTypeObject)
    {
        FUBoundedVolumeHierarchy4RaycastIterator_PyTypeObject = (PyTypeObject *)PyType_FromSpec(
            &FUBoundedVolumeHierarchy4RaycastIterator_PyTypeSpec);
        if (!FUBoundedVolumeHierarchy4RaycastIterator_PyTypeObject){ return 0; }
    }

    FUBoundedVolumeHierarchy4RaycastIterator *iter = PyObject_New(
        FUBoundedVolumeHierarchy4RaycastIterator, FUBoundedVolumeHierarchy4RaycastIterator_PyTypeObject);
    if (!iter){ return 0; }
    ...
    {
        FUBoundedVolumeHierarchy4_ray_test_node(iter->eye, iter->inv_dir, &self->nodes[0], iter->hits, &iter->hit_count);
    }
    else
    {
        iter->hit_count = 0;
...
static PyMethodDef FUBoundedVolumeHierarchy4_PyMethodDef[] = {
    {"raycast", (PyCFunction)FUBoundedVolumeHierarchy4_raycast, METH_FASTCALL, 0},
    {0, 0, 0, 0}
};

static PyMemberDef FUBoundedVolumeHierarchy4_PyMemberDef[] = {
    {"__weaklistoffset__", T_PYSSIZET, offsetof(FUBoundedVolumeHierarchy4, weakreflist), READONLY},
    {0, 0, 0, 0, 0},
};

static PyGetSetDef FUBoundedVolumeHierarchy4_PyGetSetDef[] = {
    {"nodes", (getter)FUBoundedVolumeHierarchy4_nodes, 0, 0, 0},
    {0, 0, 0, 0, 0}
};

static PyType_Slot FUBoundedVolumeHierarchy4_PyType_Slots [] = {
    {Py_tp_new, (void*)FUBoundedVolumeHierarchy4__new__},
    {Py_tp_dealloc, (void*)FUBoundedVolumeHierarchy4__dealloc__},
    {Py_tp_repr, (void*)FUBoundedVolumeHierarchy4__repr__},
    {Py_tp_members, (void*)FUBoundedVolumeHierarchy4_PyMemberDef},
    {Py_tp_getset, (void*)FUBoundedVolumeHierarchy4_PyGetSetDef},
    {Py_tp_methods, (void*)FUBoundedVolumeHierarchy4_PyMethodDef},
    {0, 0},
};

static PyType_Spec FUBoundedVolumeHierarchy4_PyTypeSpec = {
    "egeometry.FUBoundedVolumeHierarchy4",
    sizeof(FUBoundedVolumeHierarchy4),
    0,
    Py_TPFLAGS_DEFAULT,
    FUBoundedVolumeHierarchy4_PyType_Slots
};

static PyTypeObject *
define_FUBoundedVolumeHierarchy4_type(PyObject *module)
{
    PyTypeObject *type = (PyTypeObject *)PyType_FromModuleAndSpec(
        module,
        &FUBoundedVolumeHierarchy4_PyTypeSpec,
        0
    );
    if (!type){ return 0; }
    // Note:
    ...
    if (PyModule_AddObject(module, "FUBoundedVolumeHierarchy4", (PyObject *)type) < 0)
    {
        Py_DECREF(type);
        return 0;
    }

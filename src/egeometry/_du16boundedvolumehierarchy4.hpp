...
#ifndef EGEOMETRY_DU16BOUNDEDVOLUMEHIERARCHY4_HPP
#define EGEOMETRY_DU16BOUNDEDVOLUMEHIERARCHY4_HPP
...
#include "_du16boundedvolumehierarchy4type.hpp"
#include "_modulestate.hpp"

static const uint16_t DU16BoundedVolumeHierarchy4_LEAF_MASK (uint16_t(1) << (sizeof(uint16_t) * 8 - 1));

struct DU16BoundedVolumeHierarchy4Items
{
    size_t count;
    double *min_x;
    double *min_y;
...
static DU16BoundedVolumeHierarchy4Items
DU16BoundedVolumeHierarchy4_alloc_items_from_py_sequence_bounding_box_3d(
    PyObject *py_items
)
{
    EMathApi_GetDVector3ValuePointer get_vector_ptr = 0;
    char *item_data = 0;
    PyObject *py_item = 0;
    PyObject *py_position = 0;
    PyObject *py_extent = 0;
    DU16BoundedVolumeHierarchy4Items items;
    items.count = 0;
    items.min_x = 0;

    ModuleState *module_state = get_module_state();
    if (!module_state){ goto error; }
    get_vector_ptr = module_state->emath_api->DVector3_GetValuePointer;
...
static void
DU16BoundedVolumeHierarchy4_free_items(DU16BoundedVolumeHierarchy4Items items)
{
    if (items.min_x == 0)
    {
        free(items.min_x);
...
static void
DU16BoundedVolumeHierarchy4_alloc_nodes(
    DU16BoundedVolumeHierarchy4Items items,
...
)
{
    ...
    if (items.count == 0)
    {
        *nodes = (DU16BoundedVolumeHierarchy4Node *)malloc(1);
        if (!nodes)
        {
            PyErr_NoMemory();
            return;
    ...
    size_t max_item_count = (size_t)(uint16_t)(~DU16BoundedVolumeHierarchy4_LEAF_MASK) + 1;
    ...
    double *center_y = 0;
    double *center_z = 0;
    DU16BoundedVolumeHierarchy4Node *node_array = 0;
    ...
    node_array = (DU16BoundedVolumeHierarchy4Node *)malloc(sizeof(DU16BoundedVolumeHierarchy4Node) * node_array_count);
    if (!node_array){ PyErr_NoMemory(); goto error; }
    memset(node_array, 0, sizeof(DU16BoundedVolumeHierarchy4Node) * node_array_count);
...

    {
        ...
        auto fill_child = [&](size_t node_idx, int c, size_t child_start, size_t child_end) -> bool
        {
            ...
            else if (child_count_items == 1)
            {
                ...
                node_array[node_idx].child[c] = item_idx | DU16BoundedVolumeHierarchy4_LEAF_MASK;
                return true;
            }
            else
            {
        ...
        {
            DU16BoundedVolumeHierarchy4Node *shrunk = (DU16BoundedVolumeHierarchy4Node *)realloc(node_array, sizeof(DU16BoundedVolumeHierarchy4Node) * next_node);
...
static void
DU16BoundedVolumeHierarchy4_free_nodes(DU16BoundedVolumeHierarchy4Node *nodes)
{
    if (nodes)
    {
        free(nodes);
...
static PyObject *
DU16BoundedVolumeHierarchy4__new__(PyTypeObject *cls, PyObject *args, PyObject *kwds)
{
    DU16BoundedVolumeHierarchy4 *self = 0;
    DU16BoundedVolumeHierarchy4Node *nodes = 0;

    static const char *kwp[] = {"items", 0};
    PyObject *py_items = 0;
    if (!PyArg_ParseTupleAndKeywords(args, kwds, "O", (char **)kwp, &py_items)){ return 0; }

    DU16BoundedVolumeHierarchy4Items items = DU16BoundedVolumeHierarchy4_alloc_items_from_py_sequence_bounding_box_3d(py_items);
    if (items.min_x == 0){ goto error; }

    size_t node_count;
    DU16BoundedVolumeHierarchy4_alloc_nodes(items, &nodes, &node_count);
    if (!nodes && items.count > 0){ goto error; }

    DU16BoundedVolumeHierarchy4_free_items(items);

    self = (DU16BoundedVolumeHierarchy4*)cls->tp_alloc(cls, 0);
    if (!self)
    {
        goto error;
    }
...
error:
    DU16BoundedVolumeHierarchy4_free_nodes(nodes);
    DU16BoundedVolumeHierarchy4_free_items(items);
    return 0;
}

static void
DU16BoundedVolumeHierarchy4__dealloc__(DU16BoundedVolumeHierarchy4 *self)
{
    if (self->weakreflist)
    {
...
static PyObject *
DU16BoundedVolumeHierarchy4__repr__(DU16BoundedVolumeHierarchy4 *self)
{
...
static PyObject *
DU16BoundedVolumeHierarchy4_nodes(DU16BoundedVolumeHierarchy4 *self, void *closure)
{
    PyObject *bbox_cls = 0;
    PyObject *result = 0;
    ...
    {
        DU16BoundedVolumeHierarchy4Node *node = &self->nodes[n];
        ...
        for (int c = 0; c < 4; c++)
        {
            ...
            if (child_val & DU16BoundedVolumeHierarchy4_LEAF_MASK)
            {
                // leaf node - just emit the item index
                uint16_t item_idx = child_val & ~DU16BoundedVolumeHierarchy4_LEAF_MASK;
...
struct DU16BoundedVolumeHierarchy4RaycastIterator
{
    PyObject_HEAD
    DU16BoundedVolumeHierarchy4 *bvh;
    double eye[3];
    double inv_dir[3];
...
static void
DU16BoundedVolumeHierarchy4RaycastIterator__dealloc__(DU16BoundedVolumeHierarchy4RaycastIterator *self)
{
    Py_XDECREF(self->bvh);
    if (self->stack)
    {
...
static inline void
DU16BoundedVolumeHierarchy4_ray_test_node(
    const double *eye,
    const double *inv_dir,
    const DU16BoundedVolumeHierarchy4Node *node,
    uint16_t *hits,
    int *hit_count
)
{
...
static PyObject *
DU16BoundedVolumeHierarchy4RaycastIterator__next__(DU16BoundedVolumeHierarchy4RaycastIterator *self)
{
    DU16BoundedVolumeHierarchy4 *bvh = self->bvh;
    ...
    while (true)
    {
        ...
        DU16BoundedVolumeHierarchy4Node *node = &bvh->nodes[node_idx];

        // test all children of this node
        DU16BoundedVolumeHierarchy4_ray_test_node(self->eye, self->inv_dir, node, self->hits, &self->hit_count);
        self->hit_index = 0;
    }
}

static PyObject *
DU16BoundedVolumeHierarchy4RaycastIterator__iter__(DU16BoundedVolumeHierarchy4RaycastIterator *self)
{
    Py_INCREF(self);
    return (PyObject *)self;
}

static PyType_Slot DU16BoundedVolumeHierarchy4RaycastIterator_PyType_Slots[] = {
    {Py_tp_dealloc, (void *)DU16BoundedVolumeHierarchy4RaycastIterator__dealloc__},
    {Py_tp_iter, (void *)DU16BoundedVolumeHierarchy4RaycastIterator__iter__},
    {Py_tp_iternext, (void *)DU16BoundedVolumeHierarchy4RaycastIterator__next__},
    {0, 0},
};

static PyType_Spec DU16BoundedVolumeHierarchy4RaycastIterator_PyTypeSpec = {
    "egeometry.DU16BoundedVolumeHierarchy4RaycastIterator",
    sizeof(DU16BoundedVolumeHierarchy4RaycastIterator),
    0,
    Py_TPFLAGS_DEFAULT,
    DU16BoundedVolumeHierarchy4RaycastIterator_PyType_Slots
};

static PyTypeObject *DU16BoundedVolumeHierarchy4RaycastIterator_PyTypeObject = 0;

static PyObject *
DU16BoundedVolumeHierarchy4_raycast(DU16BoundedVolumeHierarchy4 *self, PyObject *const *args, Py_ssize_t nargs)
{
    if (nargs != 2)
    {
    ...
    // create iterator
    if (!DU16BoundedVolumeHierarchy4RaycastIterator_PyTypeObject)
    {
        DU16BoundedVolumeHierarchy4RaycastIterator_PyTypeObject = (PyTypeObject *)PyType_FromSpec(
            &DU16BoundedVolumeHierarchy4RaycastIterator_PyTypeSpec);
        if (!DU16BoundedVolumeHierarchy4RaycastIterator_PyTypeObject){ return 0; }
    }

    DU16BoundedVolumeHierarchy4RaycastIterator *iter = PyObject_New(
        DU16BoundedVolumeHierarchy4RaycastIterator, DU16BoundedVolumeHierarchy4RaycastIterator_PyTypeObject);
    if (!iter){ return 0; }
    ...
    {
        DU16BoundedVolumeHierarchy4_ray_test_node(iter->eye, iter->inv_dir, &self->nodes[0], iter->hits, &iter->hit_count);
    }
    else
    {
        iter->hit_count = 0;
...
static PyMethodDef DU16BoundedVolumeHierarchy4_PyMethodDef[] = {
    {"raycast", (PyCFunction)DU16BoundedVolumeHierarchy4_raycast, METH_FASTCALL, 0},
    {0, 0, 0, 0}
};

static PyMemberDef DU16BoundedVolumeHierarchy4_PyMemberDef[] = {
    {"__weaklistoffset__", T_PYSSIZET, offsetof(DU16BoundedVolumeHierarchy4, weakreflist), READONLY},
    {0, 0, 0, 0, 0},
};

static PyGetSetDef DU16BoundedVolumeHierarchy4_PyGetSetDef[] = {
    {"nodes", (getter)DU16BoundedVolumeHierarchy4_nodes, 0, 0, 0},
    {0, 0, 0, 0, 0}
};

static PyType_Slot DU16BoundedVolumeHierarchy4_PyType_Slots [] = {
    {Py_tp_new, (void*)DU16BoundedVolumeHierarchy4__new__},
    {Py_tp_dealloc, (void*)DU16BoundedVolumeHierarchy4__dealloc__},
    {Py_tp_repr, (void*)DU16BoundedVolumeHierarchy4__repr__},
    {Py_tp_members, (void*)DU16BoundedVolumeHierarchy4_PyMemberDef},
    {Py_tp_getset, (void*)DU16BoundedVolumeHierarchy4_PyGetSetDef},
    {Py_tp_methods, (void*)DU16BoundedVolumeHierarchy4_PyMethodDef},
    {0, 0},
};

static PyType_Spec DU16BoundedVolumeHierarchy4_PyTypeSpec = {
    "egeometry.DU16BoundedVolumeHierarchy4",
    sizeof(DU16BoundedVolumeHierarchy4),
    0,
    Py_TPFLAGS_DEFAULT,
    DU16BoundedVolumeHierarchy4_PyType_Slots
};

static PyTypeObject *
define_DU16BoundedVolumeHierarchy4_type(PyObject *module)
{
    PyTypeObject *type = (PyTypeObject *)PyType_FromModuleAndSpec(
        module,
        &DU16BoundedVolumeHierarchy4_PyTypeSpec,
        0
    );
    if (!type){ return 0; }
    // Note:
    ...
    if (PyModule_AddObject(module, "DU16BoundedVolumeHierarchy4", (PyObject *)type) < 0)
    {
        Py_DECREF(type);
        return 0;
    }

...
#ifndef EGEOMETRY_DUBOUNDEDVOLUMEHIERARCHY2_HPP
#define EGEOMETRY_DUBOUNDEDVOLUMEHIERARCHY2_HPP
...
#include "_duboundedvolumehierarchy2type.hpp"
#include "_modulestate.hpp"

static const unsigned int DUBoundedVolumeHierarchy2_LEAF_MASK (((unsigned int)1) << (sizeof(unsigned int) * 8 - 1));

struct DUBoundedVolumeHierarchy2Items
{
    size_t count;
    double *min_x;
    double *min_y;
...
static DUBoundedVolumeHierarchy2Items
DUBoundedVolumeHierarchy2_alloc_items_from_py_sequence_bounding_box_3d(
    PyObject *py_items
)
{
    ...
    DUBoundedVolumeHierarchy2Items items;
    items.count = 0;
    items.min_x = 0;
...
static void
DUBoundedVolumeHierarchy2_free_items(DUBoundedVolumeHierarchy2Items items)
{
    if (items.min_x == 0)
    {
        free(items.min_x);
...
static void
DUBoundedVolumeHierarchy2_alloc_nodes(
    DUBoundedVolumeHierarchy2Items items,
...
)
{
    ...
    if (items.count == 0)
    {
        *nodes = (DUBoundedVolumeHierarchy2Node *)malloc(1);
        if (!nodes)
        {
            PyErr_NoMemory();
            return;
    ...
    size_t max_item_count = (size_t)(unsigned int)(~DUBoundedVolumeHierarchy2_LEAF_MASK) + 1;
    ...
    double *center_y = 0;
    double *center_z = 0;
    DUBoundedVolumeHierarchy2Node *node_array = 0;
    ...
    node_array = (DUBoundedVolumeHierarchy2Node *)malloc(sizeof(DUBoundedVolumeHierarchy2Node) * node_array_count);
    if (!node_array){ PyErr_NoMemory(); goto error; }
    memset(node_array, 0, sizeof(DUBoundedVolumeHierarchy2Node) * node_array_count);
...

    {
        ...
        auto fill_child = [&](size_t node_idx, int c, size_t child_start, size_t child_end) -> bool
        {
            ...
            else if (child_count_items == 1)
            {
                ...
                node_array[node_idx].child[c] = item_idx | DUBoundedVolumeHierarchy2_LEAF_MASK;
                return true;
            }
            else
            {
        ...
        {
            DUBoundedVolumeHierarchy2Node *shrunk = (DUBoundedVolumeHierarchy2Node *)realloc(node_array, sizeof(DUBoundedVolumeHierarchy2Node) * next_node);
...
static void
DUBoundedVolumeHierarchy2_free_nodes(DUBoundedVolumeHierarchy2Node *nodes)
{
    if (nodes)
    {
        free(nodes);
...
static PyObject *
DUBoundedVolumeHierarchy2__new__(PyTypeObject *cls, PyObject *args, PyObject *kwds)
{
    DUBoundedVolumeHierarchy2 *self = 0;
    DUBoundedVolumeHierarchy2Node *nodes = 0;

    static const char *kwp[] = {"items", 0};
    PyObject *py_items = 0;
    if (!PyArg_ParseTupleAndKeywords(args, kwds, "O", (char **)kwp, &py_items)){ return 0; }

    DUBoundedVolumeHierarchy2Items items = DUBoundedVolumeHierarchy2_alloc_items_from_py_sequence_bounding_box_3d(py_items);
    if (items.min_x == 0){ goto error; }

    size_t node_count;
    DUBoundedVolumeHierarchy2_alloc_nodes(items, &nodes, &node_count);
    if (!nodes && items.count > 0){ goto error; }

    DUBoundedVolumeHierarchy2_free_items(items);

    self = (DUBoundedVolumeHierarchy2*)cls->tp_alloc(cls, 0);
    if (!self)
    {
        goto error;
    }
...
error:
    DUBoundedVolumeHierarchy2_free_nodes(nodes);
    DUBoundedVolumeHierarchy2_free_items(items);
    return 0;
}

static void
DUBoundedVolumeHierarchy2__dealloc__(DUBoundedVolumeHierarchy2 *self)
{
    if (self->weakreflist)
    {
...
static PyObject *
DUBoundedVolumeHierarchy2__repr__(DUBoundedVolumeHierarchy2 *self)
{
...
static PyObject *
DUBoundedVolumeHierarchy2_nodes(DUBoundedVolumeHierarchy2 *self, void *closure)
{
    PyObject *bbox_cls = 0;
    PyObject *result = 0;
    ...
    {
        DUBoundedVolumeHierarchy2Node *node = &self->nodes[n];
        ...
        for (int c = 0; c < 2; c++)
        {
            ...
            if (child_val & DUBoundedVolumeHierarchy2_LEAF_MASK)
            {
                // leaf node - just emit the item index
                unsigned int item_idx = child_val & ~DUBoundedVolumeHierarchy2_LEAF_MASK;
...
struct DUBoundedVolumeHierarchy2RaycastIterator
{
    PyObject_HEAD
    DUBoundedVolumeHierarchy2 *bvh;
    double eye[3];
    double inv_dir[3];
...
static void
DUBoundedVolumeHierarchy2RaycastIterator__dealloc__(DUBoundedVolumeHierarchy2RaycastIterator *self)
{
    Py_XDECREF(self->bvh);
    if (self->stack)
    {
...
static inline void
DUBoundedVolumeHierarchy2_ray_test_node(
    const double *eye,
    const double *inv_dir,
    const DUBoundedVolumeHierarchy2Node *node,
    unsigned int *hits,
    int *hit_count
)
{
...
static PyObject *
DUBoundedVolumeHierarchy2RaycastIterator__next__(DUBoundedVolumeHierarchy2RaycastIterator *self)
{
    DUBoundedVolumeHierarchy2 *bvh = self->bvh;
    ...
    while (true)
    {
        ...
        DUBoundedVolumeHierarchy2Node *node = &bvh->nodes[node_idx];

        // test all children of this node
        DUBoundedVolumeHierarchy2_ray_test_node(self->eye, self->inv_dir, node, self->hits, &self->hit_count);
        self->hit_index = 0;
    }
}

static PyObject *
DUBoundedVolumeHierarchy2RaycastIterator__iter__(DUBoundedVolumeHierarchy2RaycastIterator *self)
{
    Py_INCREF(self);
    return (PyObject *)self;
}

static PyType_Slot DUBoundedVolumeHierarchy2RaycastIterator_PyType_Slots[] = {
    {Py_tp_dealloc, (void *)DUBoundedVolumeHierarchy2RaycastIterator__dealloc__},
    {Py_tp_iter, (void *)DUBoundedVolumeHierarchy2RaycastIterator__iter__},
    {Py_tp_iternext, (void *)DUBoundedVolumeHierarchy2RaycastIterator__next__},
    {0, 0},
};

static PyType_Spec DUBoundedVolumeHierarchy2RaycastIterator_PyTypeSpec = {
    "egeometry.DUBoundedVolumeHierarchy2RaycastIterator",
    sizeof(DUBoundedVolumeHierarchy2RaycastIterator),
    0,
    Py_TPFLAGS_DEFAULT,
    DUBoundedVolumeHierarchy2RaycastIterator_PyType_Slots
};

static PyTypeObject *DUBoundedVolumeHierarchy2RaycastIterator_PyTypeObject = 0;

static PyObject *
DUBoundedVolumeHierarchy2_raycast(DUBoundedVolumeHierarchy2 *self, PyObject *const *args, Py_ssize_t nargs)
{
    if (nargs != 2)
    {
    ...
    // create iterator
    if (!DUBoundedVolumeHierarchy2RaycastIterator_PyTypeObject)
    {
        DUBoundedVolumeHierarchy2RaycastIterator_PyTypeObject = (PyTypeObject *)PyType_FromSpec(
            &DUBoundedVolumeHierarchy2RaycastIterator_PyTypeSpec);
        if (!DUBoundedVolumeHierarchy2RaycastIterator_PyTypeObject){ return 0; }
    }

    DUBoundedVolumeHierarchy2RaycastIterator *iter = PyObject_New(
        DUBoundedVolumeHierarchy2RaycastIterator, DUBoundedVolumeHierarchy2RaycastIterator_PyTypeObject);
    if (!iter){ return 0; }
    ...
    {
        DUBoundedVolumeHierarchy2_ray_test_node(iter->eye, iter->inv_dir, &self->nodes[0], iter->hits, &iter->hit_count);
    }
    else
    {
        iter->hit_count = 0;
...
static PyMethodDef DUBoundedVolumeHierarchy2_PyMethodDef[] = {
    {"raycast", (PyCFunction)DUBoundedVolumeHierarchy2_raycast, METH_FASTCALL, 0},
    {0, 0, 0, 0}
};

static PyMemberDef DUBoundedVolumeHierarchy2_PyMemberDef[] = {
    {"__weaklistoffset__", T_PYSSIZET, offsetof(DUBoundedVolumeHierarchy2, weakreflist), READONLY},
    {0, 0, 0, 0, 0},
};

static PyGetSetDef DUBoundedVolumeHierarchy2_PyGetSetDef[] = {
    {"nodes", (getter)DUBoundedVolumeHierarchy2_nodes, 0, 0, 0},
    {0, 0, 0, 0, 0}
};

static PyType_Slot DUBoundedVolumeHierarchy2_PyType_Slots [] = {
    {Py_tp_new, (void*)DUBoundedVolumeHierarchy2__new__},
    {Py_tp_dealloc, (void*)DUBoundedVolumeHierarchy2__dealloc__},
    {Py_tp_repr, (void*)DUBoundedVolumeHierarchy2__repr__},
    {Py_tp_members, (void*)DUBoundedVolumeHierarchy2_PyMemberDef},
    {Py_tp_getset, (void*)DUBoundedVolumeHierarchy2_PyGetSetDef},
    {Py_tp_methods, (void*)DUBoundedVolumeHierarchy2_PyMethodDef},
    {0, 0},
};

static PyType_Spec DUBoundedVolumeHierarchy2_PyTypeSpec = {
    "egeometry.DUBoundedVolumeHierarchy2",
    sizeof(DUBoundedVolumeHierarchy2),
    0,
    Py_TPFLAGS_DEFAULT,
    DUBoundedVolumeHierarchy2_PyType_Slots
};

static PyTypeObject *
define_DUBoundedVolumeHierarchy2_type(PyObject *module)
{
    PyTypeObject *type = (PyTypeObject *)PyType_FromModuleAndSpec(
        module,
        &DUBoundedVolumeHierarchy2_PyTypeSpec,
        0
    );
    if (!type){ return 0; }
    // Note:
    ...
    if (PyModule_AddObject(module, "DUBoundedVolumeHierarchy2", (PyObject *)type) < 0)
    {
        Py_DECREF(type);
        return 0;
    }

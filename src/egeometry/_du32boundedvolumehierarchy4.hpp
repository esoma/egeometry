...
#ifndef EGEOMETRY_DU32BOUNDEDVOLUMEHIERARCHY4_HPP
#define EGEOMETRY_DU32BOUNDEDVOLUMEHIERARCHY4_HPP
...
#include "_du32boundedvolumehierarchy4type.hpp"
#include "_modulestate.hpp"

static const uint32_t DU32BoundedVolumeHierarchy4_LEAF_MASK (((uint32_t)1) << (sizeof(uint32_t) * 8 - 1));

struct DU32BoundedVolumeHierarchy4Items
{
    size_t count;
    double *min_x;
    double *min_y;
...
static DU32BoundedVolumeHierarchy4Items
DU32BoundedVolumeHierarchy4_alloc_items_from_py_sequence_bounding_box_3d(
    PyObject *py_items
)
{
    ...
    DU32BoundedVolumeHierarchy4Items items;
    items.count = 0;
    items.min_x = 0;
...
static void
DU32BoundedVolumeHierarchy4_free_items(DU32BoundedVolumeHierarchy4Items items)
{
    if (items.min_x == 0)
    {
        free(items.min_x);
...
static void
DU32BoundedVolumeHierarchy4_alloc_nodes(
    DU32BoundedVolumeHierarchy4Items items,
...
)
{
    ...
    if (items.count == 0)
    {
        *nodes = (DU32BoundedVolumeHierarchy4Node *)malloc(1);
        if (!nodes)
        {
            PyErr_NoMemory();
            return;
    ...
    size_t max_item_count = (size_t)(uint32_t)(~DU32BoundedVolumeHierarchy4_LEAF_MASK) + 1;
    ...
    double *center_y = 0;
    double *center_z = 0;
    DU32BoundedVolumeHierarchy4Node *node_array = 0;
    ...
    node_array = (DU32BoundedVolumeHierarchy4Node *)malloc(sizeof(DU32BoundedVolumeHierarchy4Node) * node_array_count);
    if (!node_array){ PyErr_NoMemory(); goto error; }
    memset(node_array, 0, sizeof(DU32BoundedVolumeHierarchy4Node) * node_array_count);
...

    {
        ...
        auto fill_child = [&](size_t node_idx, int c, size_t child_start, size_t child_end) -> bool
        {
            ...
            else if (child_count_items == 1)
            {
                ...
                node_array[node_idx].child[c] = item_idx | DU32BoundedVolumeHierarchy4_LEAF_MASK;
                return true;
            }
            else
            {
        ...
        {
            DU32BoundedVolumeHierarchy4Node *shrunk = (DU32BoundedVolumeHierarchy4Node *)realloc(node_array, sizeof(DU32BoundedVolumeHierarchy4Node) * next_node);
...
static void
DU32BoundedVolumeHierarchy4_free_nodes(DU32BoundedVolumeHierarchy4Node *nodes)
{
    if (nodes)
    {
        free(nodes);
...
static PyObject *
DU32BoundedVolumeHierarchy4__new__(PyTypeObject *cls, PyObject *args, PyObject *kwds)
{
    DU32BoundedVolumeHierarchy4 *self = 0;
    DU32BoundedVolumeHierarchy4Node *nodes = 0;

    static const char *kwp[] = {"items", 0};
    PyObject *py_items = 0;
    if (!PyArg_ParseTupleAndKeywords(args, kwds, "O", (char **)kwp, &py_items)){ return 0; }

    DU32BoundedVolumeHierarchy4Items items = DU32BoundedVolumeHierarchy4_alloc_items_from_py_sequence_bounding_box_3d(py_items);
    if (items.min_x == 0){ goto error; }

    size_t node_count;
    DU32BoundedVolumeHierarchy4_alloc_nodes(items, &nodes, &node_count);
    if (!nodes && items.count > 0){ goto error; }

    DU32BoundedVolumeHierarchy4_free_items(items);

    self = (DU32BoundedVolumeHierarchy4*)cls->tp_alloc(cls, 0);
    if (!self)
    {
        goto error;
    }
...
error:
    DU32BoundedVolumeHierarchy4_free_nodes(nodes);
    DU32BoundedVolumeHierarchy4_free_items(items);
    return 0;
}

static void
DU32BoundedVolumeHierarchy4__dealloc__(DU32BoundedVolumeHierarchy4 *self)
{
    if (self->weakreflist)
    {
...
static PyObject *
DU32BoundedVolumeHierarchy4__repr__(DU32BoundedVolumeHierarchy4 *self)
{
...
static PyObject *
DU32BoundedVolumeHierarchy4_nodes(DU32BoundedVolumeHierarchy4 *self, void *closure)
{
    PyObject *bbox_cls = 0;
    PyObject *result = 0;
    ...
    {
        DU32BoundedVolumeHierarchy4Node *node = &self->nodes[n];
        ...
        for (int c = 0; c < 4; c++)
        {
            ...
            if (child_val & DU32BoundedVolumeHierarchy4_LEAF_MASK)
            {
                // leaf node - just emit the item index
                uint32_t item_idx = child_val & ~DU32BoundedVolumeHierarchy4_LEAF_MASK;
...
struct DU32BoundedVolumeHierarchy4RaycastIterator
{
    PyObject_HEAD
    DU32BoundedVolumeHierarchy4 *bvh;
    double eye[3];
    double inv_dir[3];
...
static void
DU32BoundedVolumeHierarchy4RaycastIterator__dealloc__(DU32BoundedVolumeHierarchy4RaycastIterator *self)
{
    Py_XDECREF(self->bvh);
    if (self->stack)
    {
...
static inline void
DU32BoundedVolumeHierarchy4_ray_test_node(
    const double *eye,
    const double *inv_dir,
    const DU32BoundedVolumeHierarchy4Node *node,
    uint32_t *hits,
    int *hit_count
)
{
...
static PyObject *
DU32BoundedVolumeHierarchy4RaycastIterator__next__(DU32BoundedVolumeHierarchy4RaycastIterator *self)
{
    DU32BoundedVolumeHierarchy4 *bvh = self->bvh;
    ...
    while (true)
    {
        ...
        DU32BoundedVolumeHierarchy4Node *node = &bvh->nodes[node_idx];

        // test all children of this node
        DU32BoundedVolumeHierarchy4_ray_test_node(self->eye, self->inv_dir, node, self->hits, &self->hit_count);
        self->hit_index = 0;
    }
}

static PyObject *
DU32BoundedVolumeHierarchy4RaycastIterator__iter__(DU32BoundedVolumeHierarchy4RaycastIterator *self)
{
    Py_INCREF(self);
    return (PyObject *)self;
}

static PyType_Slot DU32BoundedVolumeHierarchy4RaycastIterator_PyType_Slots[] = {
    {Py_tp_dealloc, (void *)DU32BoundedVolumeHierarchy4RaycastIterator__dealloc__},
    {Py_tp_iter, (void *)DU32BoundedVolumeHierarchy4RaycastIterator__iter__},
    {Py_tp_iternext, (void *)DU32BoundedVolumeHierarchy4RaycastIterator__next__},
    {0, 0},
};

static PyType_Spec DU32BoundedVolumeHierarchy4RaycastIterator_PyTypeSpec = {
    "egeometry.DU32BoundedVolumeHierarchy4RaycastIterator",
    sizeof(DU32BoundedVolumeHierarchy4RaycastIterator),
    0,
    Py_TPFLAGS_DEFAULT,
    DU32BoundedVolumeHierarchy4RaycastIterator_PyType_Slots
};

static PyTypeObject *DU32BoundedVolumeHierarchy4RaycastIterator_PyTypeObject = 0;

static PyObject *
DU32BoundedVolumeHierarchy4_raycast(DU32BoundedVolumeHierarchy4 *self, PyObject *const *args, Py_ssize_t nargs)
{
    if (nargs != 2)
    {
    ...
    // create iterator
    if (!DU32BoundedVolumeHierarchy4RaycastIterator_PyTypeObject)
    {
        DU32BoundedVolumeHierarchy4RaycastIterator_PyTypeObject = (PyTypeObject *)PyType_FromSpec(
            &DU32BoundedVolumeHierarchy4RaycastIterator_PyTypeSpec);
        if (!DU32BoundedVolumeHierarchy4RaycastIterator_PyTypeObject){ return 0; }
    }

    DU32BoundedVolumeHierarchy4RaycastIterator *iter = PyObject_New(
        DU32BoundedVolumeHierarchy4RaycastIterator, DU32BoundedVolumeHierarchy4RaycastIterator_PyTypeObject);
    if (!iter){ return 0; }
    ...
    {
        DU32BoundedVolumeHierarchy4_ray_test_node(iter->eye, iter->inv_dir, &self->nodes[0], iter->hits, &iter->hit_count);
    }
    else
    {
        iter->hit_count = 0;
...
static PyMethodDef DU32BoundedVolumeHierarchy4_PyMethodDef[] = {
    {"raycast", (PyCFunction)DU32BoundedVolumeHierarchy4_raycast, METH_FASTCALL, 0},
    {0, 0, 0, 0}
};

static PyMemberDef DU32BoundedVolumeHierarchy4_PyMemberDef[] = {
    {"__weaklistoffset__", T_PYSSIZET, offsetof(DU32BoundedVolumeHierarchy4, weakreflist), READONLY},
    {0, 0, 0, 0, 0},
};

static PyGetSetDef DU32BoundedVolumeHierarchy4_PyGetSetDef[] = {
    {"nodes", (getter)DU32BoundedVolumeHierarchy4_nodes, 0, 0, 0},
    {0, 0, 0, 0, 0}
};

static PyType_Slot DU32BoundedVolumeHierarchy4_PyType_Slots [] = {
    {Py_tp_new, (void*)DU32BoundedVolumeHierarchy4__new__},
    {Py_tp_dealloc, (void*)DU32BoundedVolumeHierarchy4__dealloc__},
    {Py_tp_repr, (void*)DU32BoundedVolumeHierarchy4__repr__},
    {Py_tp_members, (void*)DU32BoundedVolumeHierarchy4_PyMemberDef},
    {Py_tp_getset, (void*)DU32BoundedVolumeHierarchy4_PyGetSetDef},
    {Py_tp_methods, (void*)DU32BoundedVolumeHierarchy4_PyMethodDef},
    {0, 0},
};

static PyType_Spec DU32BoundedVolumeHierarchy4_PyTypeSpec = {
    "egeometry.DU32BoundedVolumeHierarchy4",
    sizeof(DU32BoundedVolumeHierarchy4),
    0,
    Py_TPFLAGS_DEFAULT,
    DU32BoundedVolumeHierarchy4_PyType_Slots
};

static PyTypeObject *
define_DU32BoundedVolumeHierarchy4_type(PyObject *module)
{
    PyTypeObject *type = (PyTypeObject *)PyType_FromModuleAndSpec(
        module,
        &DU32BoundedVolumeHierarchy4_PyTypeSpec,
        0
    );
    if (!type){ return 0; }
    // Note:
    ...
    if (PyModule_AddObject(module, "DU32BoundedVolumeHierarchy4", (PyObject *)type) < 0)
    {
        Py_DECREF(type);
        return 0;
    }

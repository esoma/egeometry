...
#ifndef EGEOMETRY_FUBOUNDEDVOLUMEHIERARCHY2_HPP
#define EGEOMETRY_FUBOUNDEDVOLUMEHIERARCHY2_HPP
...
#include "_fuboundedvolumehierarchy2type.hpp"
#include "_modulestate.hpp"

static const unsigned int FUBoundedVolumeHierarchy2_LEAF_MASK (((unsigned int)1) << (sizeof(unsigned int) * 8 - 1));

struct FUBoundedVolumeHierarchy2Items
{
    size_t count;
    float *min_x;
    float *min_y;
...
static FUBoundedVolumeHierarchy2Items
FUBoundedVolumeHierarchy2_alloc_items_from_py_sequence_bounding_box_3d(
    PyObject *py_items
)
{
    ...
    FUBoundedVolumeHierarchy2Items items;
    items.count = 0;
    items.min_x = 0;
...
static void
FUBoundedVolumeHierarchy2_free_items(FUBoundedVolumeHierarchy2Items items)
{
    if (items.min_x == 0)
    {
        free(items.min_x);
...
static void
FUBoundedVolumeHierarchy2_alloc_nodes(
    FUBoundedVolumeHierarchy2Items items,
...
)
{
    ...
    if (items.count == 0)
    {
        *nodes = (FUBoundedVolumeHierarchy2Node *)malloc(1);
        if (!nodes)
        {
            PyErr_NoMemory();
            return;
    ...
    size_t max_item_count = (size_t)(unsigned int)(~FUBoundedVolumeHierarchy2_LEAF_MASK) + 1;
    ...
    float *center_y = 0;
    float *center_z = 0;
    FUBoundedVolumeHierarchy2Node *node_array = 0;
    ...
    node_array = (FUBoundedVolumeHierarchy2Node *)malloc(sizeof(FUBoundedVolumeHierarchy2Node) * node_array_count);
    if (!node_array){ PyErr_NoMemory(); goto error; }
    memset(node_array, 0, sizeof(FUBoundedVolumeHierarchy2Node) * node_array_count);
...

    {
        ...
        auto fill_child = [&](size_t node_idx, int c, size_t child_start, size_t child_end) -> bool
        {
            ...
            else if (child_count_items == 1)
            {
                ...
                node_array[node_idx].child[c] = item_idx | FUBoundedVolumeHierarchy2_LEAF_MASK;
                return true;
            }
            else
            {
        ...
        {
            FUBoundedVolumeHierarchy2Node *shrunk = (FUBoundedVolumeHierarchy2Node *)realloc(node_array, sizeof(FUBoundedVolumeHierarchy2Node) * next_node);
...
static void
FUBoundedVolumeHierarchy2_free_nodes(FUBoundedVolumeHierarchy2Node *nodes)
{
    if (nodes)
    {
        free(nodes);
...
static PyObject *
FUBoundedVolumeHierarchy2__new__(PyTypeObject *cls, PyObject *args, PyObject *kwds)
{
    FUBoundedVolumeHierarchy2 *self = 0;
    FUBoundedVolumeHierarchy2Node *nodes = 0;

    static const char *kwp[] = {"items", 0};
    PyObject *py_items = 0;
    if (!PyArg_ParseTupleAndKeywords(args, kwds, "O", (char **)kwp, &py_items)){ return 0; }

    FUBoundedVolumeHierarchy2Items items = FUBoundedVolumeHierarchy2_alloc_items_from_py_sequence_bounding_box_3d(py_items);
    if (items.min_x == 0){ goto error; }

    size_t node_count;
    FUBoundedVolumeHierarchy2_alloc_nodes(items, &nodes, &node_count);
    if (!nodes && items.count > 0){ goto error; }

    FUBoundedVolumeHierarchy2_free_items(items);

    self = (FUBoundedVolumeHierarchy2*)cls->tp_alloc(cls, 0);
    if (!self)
    {
        goto error;
    }
...
error:
    FUBoundedVolumeHierarchy2_free_nodes(nodes);
    FUBoundedVolumeHierarchy2_free_items(items);
    return 0;
}

static void
FUBoundedVolumeHierarchy2__dealloc__(FUBoundedVolumeHierarchy2 *self)
{
    if (self->weakreflist)
    {
...
static PyObject *
FUBoundedVolumeHierarchy2__repr__(FUBoundedVolumeHierarchy2 *self)
{
...
static PyObject *
FUBoundedVolumeHierarchy2_nodes(FUBoundedVolumeHierarchy2 *self, void *closure)
{
    PyObject *bbox_cls = 0;
    PyObject *result = 0;
    ...
    {
        FUBoundedVolumeHierarchy2Node *node = &self->nodes[n];
        ...
        for (int c = 0; c < 2; c++)
        {
            ...
            if (child_val & FUBoundedVolumeHierarchy2_LEAF_MASK)
            {
                // leaf node - just emit the item index
                unsigned int item_idx = child_val & ~FUBoundedVolumeHierarchy2_LEAF_MASK;
...
struct FUBoundedVolumeHierarchy2RaycastIterator
{
    PyObject_HEAD
    FUBoundedVolumeHierarchy2 *bvh;
    float eye[3];
    float inv_dir[3];
...
static void
FUBoundedVolumeHierarchy2RaycastIterator__dealloc__(FUBoundedVolumeHierarchy2RaycastIterator *self)
{
    Py_XDECREF(self->bvh);
    if (self->stack)
    {
...
static inline void
FUBoundedVolumeHierarchy2_ray_test_node(
    const float *eye,
    const float *inv_dir,
    const FUBoundedVolumeHierarchy2Node *node,
    unsigned int *hits,
    int *hit_count
)
{
...
static PyObject *
FUBoundedVolumeHierarchy2RaycastIterator__next__(FUBoundedVolumeHierarchy2RaycastIterator *self)
{
    FUBoundedVolumeHierarchy2 *bvh = self->bvh;
    ...
    while (true)
    {
        ...
        FUBoundedVolumeHierarchy2Node *node = &bvh->nodes[node_idx];

        // test all children of this node
        FUBoundedVolumeHierarchy2_ray_test_node(self->eye, self->inv_dir, node, self->hits, &self->hit_count);
        self->hit_index = 0;
    }
}

static PyObject *
FUBoundedVolumeHierarchy2RaycastIterator__iter__(FUBoundedVolumeHierarchy2RaycastIterator *self)
{
    Py_INCREF(self);
    return (PyObject *)self;
}

static PyType_Slot FUBoundedVolumeHierarchy2RaycastIterator_PyType_Slots[] = {
    {Py_tp_dealloc, (void *)FUBoundedVolumeHierarchy2RaycastIterator__dealloc__},
    {Py_tp_iter, (void *)FUBoundedVolumeHierarchy2RaycastIterator__iter__},
    {Py_tp_iternext, (void *)FUBoundedVolumeHierarchy2RaycastIterator__next__},
    {0, 0},
};

static PyType_Spec FUBoundedVolumeHierarchy2RaycastIterator_PyTypeSpec = {
    "egeometry.FUBoundedVolumeHierarchy2RaycastIterator",
    sizeof(FUBoundedVolumeHierarchy2RaycastIterator),
    0,
    Py_TPFLAGS_DEFAULT,
    FUBoundedVolumeHierarchy2RaycastIterator_PyType_Slots
};

static PyTypeObject *FUBoundedVolumeHierarchy2RaycastIterator_PyTypeObject = 0;

static PyObject *
FUBoundedVolumeHierarchy2_raycast(FUBoundedVolumeHierarchy2 *self, PyObject *const *args, Py_ssize_t nargs)
{
    if (nargs != 2)
    {
    ...
    // create iterator
    if (!FUBoundedVolumeHierarchy2RaycastIterator_PyTypeObject)
    {
        FUBoundedVolumeHierarchy2RaycastIterator_PyTypeObject = (PyTypeObject *)PyType_FromSpec(
            &FUBoundedVolumeHierarchy2RaycastIterator_PyTypeSpec);
        if (!FUBoundedVolumeHierarchy2RaycastIterator_PyTypeObject){ return 0; }
    }

    FUBoundedVolumeHierarchy2RaycastIterator *iter = PyObject_New(
        FUBoundedVolumeHierarchy2RaycastIterator, FUBoundedVolumeHierarchy2RaycastIterator_PyTypeObject);
    if (!iter){ return 0; }
    ...
    {
        FUBoundedVolumeHierarchy2_ray_test_node(iter->eye, iter->inv_dir, &self->nodes[0], iter->hits, &iter->hit_count);
    }
    else
    {
        iter->hit_count = 0;
...
static PyMethodDef FUBoundedVolumeHierarchy2_PyMethodDef[] = {
    {"raycast", (PyCFunction)FUBoundedVolumeHierarchy2_raycast, METH_FASTCALL, 0},
    {0, 0, 0, 0}
};

static PyMemberDef FUBoundedVolumeHierarchy2_PyMemberDef[] = {
    {"__weaklistoffset__", T_PYSSIZET, offsetof(FUBoundedVolumeHierarchy2, weakreflist), READONLY},
    {0, 0, 0, 0, 0},
};

static PyGetSetDef FUBoundedVolumeHierarchy2_PyGetSetDef[] = {
    {"nodes", (getter)FUBoundedVolumeHierarchy2_nodes, 0, 0, 0},
    {0, 0, 0, 0, 0}
};

static PyType_Slot FUBoundedVolumeHierarchy2_PyType_Slots [] = {
    {Py_tp_new, (void*)FUBoundedVolumeHierarchy2__new__},
    {Py_tp_dealloc, (void*)FUBoundedVolumeHierarchy2__dealloc__},
    {Py_tp_repr, (void*)FUBoundedVolumeHierarchy2__repr__},
    {Py_tp_members, (void*)FUBoundedVolumeHierarchy2_PyMemberDef},
    {Py_tp_getset, (void*)FUBoundedVolumeHierarchy2_PyGetSetDef},
    {Py_tp_methods, (void*)FUBoundedVolumeHierarchy2_PyMethodDef},
    {0, 0},
};

static PyType_Spec FUBoundedVolumeHierarchy2_PyTypeSpec = {
    "egeometry.FUBoundedVolumeHierarchy2",
    sizeof(FUBoundedVolumeHierarchy2),
    0,
    Py_TPFLAGS_DEFAULT,
    FUBoundedVolumeHierarchy2_PyType_Slots
};

static PyTypeObject *
define_FUBoundedVolumeHierarchy2_type(PyObject *module)
{
    PyTypeObject *type = (PyTypeObject *)PyType_FromModuleAndSpec(
        module,
        &FUBoundedVolumeHierarchy2_PyTypeSpec,
        0
    );
    if (!type){ return 0; }
    // Note:
    ...
    if (PyModule_AddObject(module, "FUBoundedVolumeHierarchy2", (PyObject *)type) < 0)
    {
        Py_DECREF(type);
        return 0;
    }

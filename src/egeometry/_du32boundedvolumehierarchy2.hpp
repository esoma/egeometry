...
#ifndef EGEOMETRY_DU32BOUNDEDVOLUMEHIERARCHY2_HPP
#define EGEOMETRY_DU32BOUNDEDVOLUMEHIERARCHY2_HPP
...
#include "_du32boundedvolumehierarchy2type.hpp"
#include "_modulestate.hpp"

static const uint32_t DU32BoundedVolumeHierarchy2_LEAF_MASK (((uint32_t)1) << (sizeof(uint32_t) * 8 - 1));

struct DU32BoundedVolumeHierarchy2Items
{
    size_t count;
    double *min_x;
    double *min_y;
...
static DU32BoundedVolumeHierarchy2Items
DU32BoundedVolumeHierarchy2_alloc_items_from_py_sequence_bounding_box_3d(
    PyObject *py_items
)
{
    ...
    DU32BoundedVolumeHierarchy2Items items;
    items.count = 0;
    items.min_x = 0;
...
static void
DU32BoundedVolumeHierarchy2_free_items(DU32BoundedVolumeHierarchy2Items items)
{
    if (items.min_x == 0)
    {
        free(items.min_x);
...
static void
DU32BoundedVolumeHierarchy2_alloc_nodes(
    DU32BoundedVolumeHierarchy2Items items,
    DU32BoundedVolumeHierarchy2Node **nodes,
    size_t *node_count
)
{
    ...
    if (items.count == 0)
    {
        *nodes = (DU32BoundedVolumeHierarchy2Node *)malloc(1);
        if (!nodes)
        {
            PyErr_NoMemory();
            return;
    ...
    size_t max_item_count = (size_t)(uint32_t)(~DU32BoundedVolumeHierarchy2_LEAF_MASK) + 1;
    ...
    double *center_y = 0;
    double *center_z = 0;
    DU32BoundedVolumeHierarchy2Node *node_array = 0;
    ...
    node_array = (DU32BoundedVolumeHierarchy2Node *)malloc(sizeof(DU32BoundedVolumeHierarchy2Node) * node_array_count);
    if (!node_array){ PyErr_NoMemory(); goto error; }
    memset(node_array, 0, sizeof(DU32BoundedVolumeHierarchy2Node) * node_array_count);
...

    {
        ...
        auto fill_child = [&](size_t node_idx, int c, size_t child_start, size_t child_end) -> bool
        {
            ...
            else if (child_count_items == 1)
            {
                ...
                node_array[node_idx].child[c] = item_idx | DU32BoundedVolumeHierarchy2_LEAF_MASK;
                return true;
            }
            else
            {
        ...
        {
            DU32BoundedVolumeHierarchy2Node *shrunk = (DU32BoundedVolumeHierarchy2Node *)realloc(node_array, sizeof(DU32BoundedVolumeHierarchy2Node) * next_node);
...
static void
DU32BoundedVolumeHierarchy2_free_nodes(DU32BoundedVolumeHierarchy2Node *nodes)
{
    if (nodes)
    {
        free(nodes);
...
static PyObject *
DU32BoundedVolumeHierarchy2__new__(PyTypeObject *cls, PyObject *args, PyObject *kwds)
{
    DU32BoundedVolumeHierarchy2 *self = 0;
    DU32BoundedVolumeHierarchy2Node *nodes = 0;

    static const char *kwp[] = {"items", 0};
    PyObject *py_items = 0;
    if (!PyArg_ParseTupleAndKeywords(args, kwds, "O", (char **)kwp, &py_items)){ return 0; }

    DU32BoundedVolumeHierarchy2Items items = DU32BoundedVolumeHierarchy2_alloc_items_from_py_sequence_bounding_box_3d(py_items);
    if (items.min_x == 0){ goto error; }

    size_t node_count;
    DU32BoundedVolumeHierarchy2_alloc_nodes(items, &nodes, &node_count);
    if (!nodes && items.count > 0){ goto error; }

    DU32BoundedVolumeHierarchy2_free_items(items);

    self = (DU32BoundedVolumeHierarchy2*)cls->tp_alloc(cls, 0);
    if (!self)
    {
        goto error;
    }
...
error:
    DU32BoundedVolumeHierarchy2_free_nodes(nodes);
    DU32BoundedVolumeHierarchy2_free_items(items);
    return 0;
}

static void
DU32BoundedVolumeHierarchy2__dealloc__(DU32BoundedVolumeHierarchy2 *self)
{
    if (self->weakreflist)
    {
...
static PyObject *
DU32BoundedVolumeHierarchy2__repr__(DU32BoundedVolumeHierarchy2 *self)
{
...
static PyObject *
DU32BoundedVolumeHierarchy2_nodes(DU32BoundedVolumeHierarchy2 *self, void *closure)
{
    PyObject *bbox_cls = 0;
    PyObject *result = 0;
    ...
    {
        DU32BoundedVolumeHierarchy2Node *node = &self->nodes[n];
        ...
        for (int c = 0; c < 2; c++)
        {
            ...
            if (child_val & DU32BoundedVolumeHierarchy2_LEAF_MASK)
            {
                // leaf node - just emit the item index
                uint32_t item_idx = child_val & ~DU32BoundedVolumeHierarchy2_LEAF_MASK;
...
struct DU32BoundedVolumeHierarchy2RaycastIterator
{
    PyObject_HEAD
    DU32BoundedVolumeHierarchy2 *bvh;
    double eye[3];
    double inv_dir[3];
...
static void
DU32BoundedVolumeHierarchy2RaycastIterator__dealloc__(DU32BoundedVolumeHierarchy2RaycastIterator *self)
{
    Py_XDECREF(self->bvh);
    if (self->stack)
    {
...
static inline void
DU32BoundedVolumeHierarchy2_ray_test_node(
    const double *eye,
    const double *inv_dir,
    const DU32BoundedVolumeHierarchy2Node *node,
    uint32_t *hits,
    int *hit_count
)
{
...
static PyObject *
DU32BoundedVolumeHierarchy2RaycastIterator__next__(DU32BoundedVolumeHierarchy2RaycastIterator *self)
{
    DU32BoundedVolumeHierarchy2 *bvh = self->bvh;
    ...
    while (true)
    {
        ...
        DU32BoundedVolumeHierarchy2Node *node = &bvh->nodes[node_idx];

        // test all children of this node
        DU32BoundedVolumeHierarchy2_ray_test_node(self->eye, self->inv_dir, node, self->hits, &self->hit_count);
        self->hit_index = 0;
    }
}

static PyObject *
DU32BoundedVolumeHierarchy2RaycastIterator__iter__(DU32BoundedVolumeHierarchy2RaycastIterator *self)
{
    Py_INCREF(self);
    return (PyObject *)self;
}

static PyType_Slot DU32BoundedVolumeHierarchy2RaycastIterator_PyType_Slots[] = {
    {Py_tp_dealloc, (void *)DU32BoundedVolumeHierarchy2RaycastIterator__dealloc__},
    {Py_tp_iter, (void *)DU32BoundedVolumeHierarchy2RaycastIterator__iter__},
    {Py_tp_iternext, (void *)DU32BoundedVolumeHierarchy2RaycastIterator__next__},
    {0, 0},
};

static PyType_Spec DU32BoundedVolumeHierarchy2RaycastIterator_PyTypeSpec = {
    "egeometry.DU32BoundedVolumeHierarchy2RaycastIterator",
    sizeof(DU32BoundedVolumeHierarchy2RaycastIterator),
    0,
    Py_TPFLAGS_DEFAULT,
    DU32BoundedVolumeHierarchy2RaycastIterator_PyType_Slots
};

static PyTypeObject *DU32BoundedVolumeHierarchy2RaycastIterator_PyTypeObject = 0;

static PyObject *
DU32BoundedVolumeHierarchy2_raycast(DU32BoundedVolumeHierarchy2 *self, PyObject *const *args, Py_ssize_t nargs)
{
    if (nargs != 2)
    {
    ...
    // create iterator
    if (!DU32BoundedVolumeHierarchy2RaycastIterator_PyTypeObject)
    {
        DU32BoundedVolumeHierarchy2RaycastIterator_PyTypeObject = (PyTypeObject *)PyType_FromSpec(
            &DU32BoundedVolumeHierarchy2RaycastIterator_PyTypeSpec);
        if (!DU32BoundedVolumeHierarchy2RaycastIterator_PyTypeObject){ return 0; }
    }

    DU32BoundedVolumeHierarchy2RaycastIterator *iter = PyObject_New(
        DU32BoundedVolumeHierarchy2RaycastIterator, DU32BoundedVolumeHierarchy2RaycastIterator_PyTypeObject);
    if (!iter){ return 0; }
    ...
    {
        DU32BoundedVolumeHierarchy2_ray_test_node(iter->eye, iter->inv_dir, &self->nodes[0], iter->hits, &iter->hit_count);
    }
    else
    {
        iter->hit_count = 0;
...
static PyMethodDef DU32BoundedVolumeHierarchy2_PyMethodDef[] = {
    {"raycast", (PyCFunction)DU32BoundedVolumeHierarchy2_raycast, METH_FASTCALL, 0},
    {0, 0, 0, 0}
};

static PyMemberDef DU32BoundedVolumeHierarchy2_PyMemberDef[] = {
    {"__weaklistoffset__", T_PYSSIZET, offsetof(DU32BoundedVolumeHierarchy2, weakreflist), READONLY},
    {0, 0, 0, 0, 0},
};

static PyGetSetDef DU32BoundedVolumeHierarchy2_PyGetSetDef[] = {
    {"nodes", (getter)DU32BoundedVolumeHierarchy2_nodes, 0, 0, 0},
    {0, 0, 0, 0, 0}
};

static PyType_Slot DU32BoundedVolumeHierarchy2_PyType_Slots [] = {
    {Py_tp_new, (void*)DU32BoundedVolumeHierarchy2__new__},
    {Py_tp_dealloc, (void*)DU32BoundedVolumeHierarchy2__dealloc__},
    {Py_tp_repr, (void*)DU32BoundedVolumeHierarchy2__repr__},
    {Py_tp_members, (void*)DU32BoundedVolumeHierarchy2_PyMemberDef},
    {Py_tp_getset, (void*)DU32BoundedVolumeHierarchy2_PyGetSetDef},
    {Py_tp_methods, (void*)DU32BoundedVolumeHierarchy2_PyMethodDef},
    {0, 0},
};

static PyType_Spec DU32BoundedVolumeHierarchy2_PyTypeSpec = {
    "egeometry.DU32BoundedVolumeHierarchy2",
    sizeof(DU32BoundedVolumeHierarchy2),
    0,
    Py_TPFLAGS_DEFAULT,
    DU32BoundedVolumeHierarchy2_PyType_Slots
};

static PyTypeObject *
define_DU32BoundedVolumeHierarchy2_type(PyObject *module)
{
    PyTypeObject *type = (PyTypeObject *)PyType_FromModuleAndSpec(
        module,
        &DU32BoundedVolumeHierarchy2_PyTypeSpec,
        0
    );
    if (!type){ return 0; }
    // Note:
    ...
    if (PyModule_AddObject(module, "DU32BoundedVolumeHierarchy2", (PyObject *)type) < 0)
    {
        Py_DECREF(type);
        return 0;
    }

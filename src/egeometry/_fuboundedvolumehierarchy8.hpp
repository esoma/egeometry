...
#ifndef EGEOMETRY_FUBOUNDEDVOLUMEHIERARCHY8_HPP
#define EGEOMETRY_FUBOUNDEDVOLUMEHIERARCHY8_HPP
...
static FUBoundedVolumeHierarchy8Items
FUBoundedVolumeHierarchy8_alloc_items_from_py_sequence_bounding_box_3d(
    PyObject *py_items
)
{
    ...
    FUBoundedVolumeHierarchy8Items items;
    items.count = 0;
    items.min_x = 0;
...
static void
FUBoundedVolumeHierarchy8_free_items(FUBoundedVolumeHierarchy8Items items)
{
    if (items.min_x == 0)
    {
        free(items.min_x);
...
static void
FUBoundedVolumeHierarchy8_alloc_nodes(
    FUBoundedVolumeHierarchy8Items items,
...
)
{
    ...
    if (items.count == 0)
    {
        *nodes = (FUBoundedVolumeHierarchy8Node *)malloc(1);
        if (!nodes)
        {
            PyErr_NoMemory();
            return;
    ...
    size_t max_item_count = (size_t)(unsigned int)(~FUBoundedVolumeHierarchy8_LEAF_MASK) + 1;
    ...
    float *center_y = 0;
    float *center_z = 0;
    FUBoundedVolumeHierarchy8Node *node_array = 0;
    ...
    node_array = (FUBoundedVolumeHierarchy8Node *)malloc(sizeof(FUBoundedVolumeHierarchy8Node) * node_array_count);
    if (!node_array){ PyErr_NoMemory(); goto error; }
    memset(node_array, 0, sizeof(FUBoundedVolumeHierarchy8Node) * node_array_count);
...

    {
        ...
        auto fill_child = [&](size_t node_idx, int c, size_t child_start, size_t child_end) -> bool
        {
            ...
            else if (child_count_items == 1)
            {
                ...
                node_array[node_idx].child[c] = item_idx | FUBoundedVolumeHierarchy8_LEAF_MASK;
                return true;
            }
            else
            {
        ...
        {
            FUBoundedVolumeHierarchy8Node *shrunk = (FUBoundedVolumeHierarchy8Node *)realloc(node_array, sizeof(FUBoundedVolumeHierarchy8Node) * next_node);
...
static void
FUBoundedVolumeHierarchy8_free_nodes(FUBoundedVolumeHierarchy8Node *nodes)
{
    if (nodes)
    {
        free(nodes);
...
static PyObject *
FUBoundedVolumeHierarchy8__new__(PyTypeObject *cls, PyObject *args, PyObject *kwds)
{
    FUBoundedVolumeHierarchy8 *self = 0;
    FUBoundedVolumeHierarchy8Node *nodes = 0;

    static const char *kwp[] = {"items", 0};
    PyObject *py_items = 0;
    if (!PyArg_ParseTupleAndKeywords(args, kwds, "O", (char **)kwp, &py_items)){ return 0; }

    FUBoundedVolumeHierarchy8Items items = FUBoundedVolumeHierarchy8_alloc_items_from_py_sequence_bounding_box_3d(py_items);
    if (items.min_x == 0){ goto error; }

    size_t node_count;
    FUBoundedVolumeHierarchy8_alloc_nodes(items, &nodes, &node_count);
    if (!nodes && items.count > 0){ goto error; }

    FUBoundedVolumeHierarchy8_free_items(items);

    self = (FUBoundedVolumeHierarchy8*)cls->tp_alloc(cls, 0);
    if (!self)
    {
        goto error;
    }
...
error:
    FUBoundedVolumeHierarchy8_free_nodes(nodes);
    FUBoundedVolumeHierarchy8_free_items(items);
    return 0;
}

static void
FUBoundedVolumeHierarchy8__dealloc__(FUBoundedVolumeHierarchy8 *self)
{
    if (self->weakreflist)
    {
...
static PyObject *
FUBoundedVolumeHierarchy8__repr__(FUBoundedVolumeHierarchy8 *self)
{
...
static PyObject *
FUBoundedVolumeHierarchy8_nodes(FUBoundedVolumeHierarchy8 *self, void *closure)
{
    PyObject *bbox_cls = 0;
    PyObject *result = 0;
    ...
    {
        FUBoundedVolumeHierarchy8Node *node = &self->nodes[n];
        ...
        for (int c = 0; c < 8; c++)
        {
            ...
            if (child_val & FUBoundedVolumeHierarchy8_LEAF_MASK)
            {
                // leaf node - just emit the item index
                unsigned int item_idx = child_val & ~FUBoundedVolumeHierarchy8_LEAF_MASK;
...
struct FUBoundedVolumeHierarchy8RaycastIterator
{
    PyObject_HEAD
    FUBoundedVolumeHierarchy8 *bvh;
    float eye[3];
    float inv_dir[3];
...
static void
FUBoundedVolumeHierarchy8RaycastIterator__dealloc__(FUBoundedVolumeHierarchy8RaycastIterator *self)
{
    Py_XDECREF(self->bvh);
    if (self->stack)
    {
...
static inline void
FUBoundedVolumeHierarchy8_ray_test_node(
    const float *eye,
    const float *inv_dir,
    const FUBoundedVolumeHierarchy8Node *node,
    unsigned int *hits,
    int *hit_count
)
{
...
static PyObject *
FUBoundedVolumeHierarchy8RaycastIterator__next__(FUBoundedVolumeHierarchy8RaycastIterator *self)
{
    FUBoundedVolumeHierarchy8 *bvh = self->bvh;
    ...
    while (true)
    {
        ...
        FUBoundedVolumeHierarchy8Node *node = &bvh->nodes[node_idx];

        // test all children of this node
        FUBoundedVolumeHierarchy8_ray_test_node(self->eye, self->inv_dir, node, self->hits, &self->hit_count);
        self->hit_index = 0;
    }
}

static PyObject *
FUBoundedVolumeHierarchy8RaycastIterator__iter__(FUBoundedVolumeHierarchy8RaycastIterator *self)
{
    Py_INCREF(self);
    return (PyObject *)self;
}

static PyType_Slot FUBoundedVolumeHierarchy8RaycastIterator_PyType_Slots[] = {
    {Py_tp_dealloc, (void *)FUBoundedVolumeHierarchy8RaycastIterator__dealloc__},
    {Py_tp_iter, (void *)FUBoundedVolumeHierarchy8RaycastIterator__iter__},
    {Py_tp_iternext, (void *)FUBoundedVolumeHierarchy8RaycastIterator__next__},
    {0, 0},
};

static PyType_Spec FUBoundedVolumeHierarchy8RaycastIterator_PyTypeSpec = {
    "egeometry.FUBoundedVolumeHierarchy8RaycastIterator",
    sizeof(FUBoundedVolumeHierarchy8RaycastIterator),
    0,
    Py_TPFLAGS_DEFAULT,
    FUBoundedVolumeHierarchy8RaycastIterator_PyType_Slots
};

static PyTypeObject *FUBoundedVolumeHierarchy8RaycastIterator_PyTypeObject = 0;

static PyObject *
FUBoundedVolumeHierarchy8_raycast(FUBoundedVolumeHierarchy8 *self, PyObject *const *args, Py_ssize_t nargs)
{
    if (nargs != 2)
    {
    ...
    // create iterator
    if (!FUBoundedVolumeHierarchy8RaycastIterator_PyTypeObject)
    {
        FUBoundedVolumeHierarchy8RaycastIterator_PyTypeObject = (PyTypeObject *)PyType_FromSpec(
            &FUBoundedVolumeHierarchy8RaycastIterator_PyTypeSpec);
        if (!FUBoundedVolumeHierarchy8RaycastIterator_PyTypeObject){ return 0; }
    }

    FUBoundedVolumeHierarchy8RaycastIterator *iter = PyObject_New(
        FUBoundedVolumeHierarchy8RaycastIterator, FUBoundedVolumeHierarchy8RaycastIterator_PyTypeObject);
    if (!iter){ return 0; }
    ...
    {
        FUBoundedVolumeHierarchy8_ray_test_node(iter->eye, iter->inv_dir, &self->nodes[0], iter->hits, &iter->hit_count);
    }
    else
    {
        iter->hit_count = 0;
...
static PyMethodDef FUBoundedVolumeHierarchy8_PyMethodDef[] = {
    {"raycast", (PyCFunction)FUBoundedVolumeHierarchy8_raycast, METH_FASTCALL, 0},
    {0, 0, 0, 0}
};

static PyMemberDef FUBoundedVolumeHierarchy8_PyMemberDef[] = {
    {"__weaklistoffset__", T_PYSSIZET, offsetof(FUBoundedVolumeHierarchy8, weakreflist), READONLY},
    {0, 0, 0, 0, 0},
};

static PyGetSetDef FUBoundedVolumeHierarchy8_PyGetSetDef[] = {
    {"nodes", (getter)FUBoundedVolumeHierarchy8_nodes, 0, 0, 0},
    {0, 0, 0, 0, 0}
};

static PyType_Slot FUBoundedVolumeHierarchy8_PyType_Slots [] = {
    {Py_tp_new, (void*)FUBoundedVolumeHierarchy8__new__},
    {Py_tp_dealloc, (void*)FUBoundedVolumeHierarchy8__dealloc__},
    {Py_tp_repr, (void*)FUBoundedVolumeHierarchy8__repr__},
    {Py_tp_members, (void*)FUBoundedVolumeHierarchy8_PyMemberDef},
    {Py_tp_getset, (void*)FUBoundedVolumeHierarchy8_PyGetSetDef},
    {Py_tp_methods, (void*)FUBoundedVolumeHierarchy8_PyMethodDef},
    {0, 0},
};

static PyType_Spec FUBoundedVolumeHierarchy8_PyTypeSpec = {
    "egeometry.FUBoundedVolumeHierarchy8",
    sizeof(FUBoundedVolumeHierarchy8),
    0,
    Py_TPFLAGS_DEFAULT,
    FUBoundedVolumeHierarchy8_PyType_Slots
};

static PyTypeObject *
define_FUBoundedVolumeHierarchy8_type(PyObject *module)
{
    PyTypeObject *type = (PyTypeObject *)PyType_FromModuleAndSpec(
        module,
        &FUBoundedVolumeHierarchy8_PyTypeSpec,
        0
    );
    if (!type){ return 0; }
    // Note:
    ...
    if (PyModule_AddObject(module, "FUBoundedVolumeHierarchy8", (PyObject *)type) < 0)
    {
        Py_DECREF(type);
        return 0;
    }

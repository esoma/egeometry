...
#ifndef EGEOMETRY_FU8BOUNDEDVOLUMEHIERARCHY4_HPP
#define EGEOMETRY_FU8BOUNDEDVOLUMEHIERARCHY4_HPP
...
#include "_fu8boundedvolumehierarchy4type.hpp"
#include "_modulestate.hpp"

static const uint8_t FU8BoundedVolumeHierarchy4_LEAF_MASK (uint8_t(1) << (sizeof(uint8_t) * 8 - 1));

struct FU8BoundedVolumeHierarchy4Items
{
    size_t count;
    float *min_x;
    float *min_y;
...
static FU8BoundedVolumeHierarchy4Items
FU8BoundedVolumeHierarchy4_alloc_items_from_py_sequence_bounding_box_3d(
    PyObject *py_items
)
{
    char *item_data = 0;
    ...
    FU8BoundedVolumeHierarchy4Items items;
    items.count = 0;
    items.min_x = 0;
...
static void
FU8BoundedVolumeHierarchy4_free_items(FU8BoundedVolumeHierarchy4Items items)
{
    if (items.min_x == 0)
    {
        free(items.min_x);
...
static void
FU8BoundedVolumeHierarchy4_alloc_nodes(
    FU8BoundedVolumeHierarchy4Items items,
...
)
{
    ...
    if (items.count == 0)
    {
        *nodes = (FU8BoundedVolumeHierarchy4Node *)malloc(1);
        if (!nodes)
        {
            PyErr_NoMemory();
            return;
    ...
    size_t max_item_count = (size_t)(uint8_t)(~FU8BoundedVolumeHierarchy4_LEAF_MASK) + 1;
    ...
    float *center_y = 0;
    float *center_z = 0;
    FU8BoundedVolumeHierarchy4Node *node_array = 0;
    ...
    node_array = (FU8BoundedVolumeHierarchy4Node *)malloc(sizeof(FU8BoundedVolumeHierarchy4Node) * node_array_count);
    if (!node_array){ PyErr_NoMemory(); goto error; }
    memset(node_array, 0, sizeof(FU8BoundedVolumeHierarchy4Node) * node_array_count);
...

    {
        ...
        auto fill_child = [&](size_t node_idx, int c, size_t child_start, size_t child_end) -> bool
        {
            ...
            else if (child_count_items == 1)
            {
                ...
                node_array[node_idx].child[c] = item_idx | FU8BoundedVolumeHierarchy4_LEAF_MASK;
                return true;
            }
            else
            {
        ...
        {
            FU8BoundedVolumeHierarchy4Node *shrunk = (FU8BoundedVolumeHierarchy4Node *)realloc(node_array, sizeof(FU8BoundedVolumeHierarchy4Node) * next_node);
...
static void
FU8BoundedVolumeHierarchy4_free_nodes(FU8BoundedVolumeHierarchy4Node *nodes)
{
    if (nodes)
    {
        free(nodes);
...
static PyObject *
FU8BoundedVolumeHierarchy4__new__(PyTypeObject *cls, PyObject *args, PyObject *kwds)
{
    FU8BoundedVolumeHierarchy4 *self = 0;
    FU8BoundedVolumeHierarchy4Node *nodes = 0;

    static const char *kwp[] = {"items", 0};
    PyObject *py_items = 0;
    if (!PyArg_ParseTupleAndKeywords(args, kwds, "O", (char **)kwp, &py_items)){ return 0; }

    FU8BoundedVolumeHierarchy4Items items = FU8BoundedVolumeHierarchy4_alloc_items_from_py_sequence_bounding_box_3d(py_items);
    if (items.min_x == 0){ goto error; }

    size_t node_count;
    FU8BoundedVolumeHierarchy4_alloc_nodes(items, &nodes, &node_count);
    if (!nodes && items.count > 0){ goto error; }

    FU8BoundedVolumeHierarchy4_free_items(items);

    self = (FU8BoundedVolumeHierarchy4*)cls->tp_alloc(cls, 0);
    if (!self)
    {
        goto error;
    }
...
error:
    FU8BoundedVolumeHierarchy4_free_nodes(nodes);
    FU8BoundedVolumeHierarchy4_free_items(items);
    return 0;
}

static void
FU8BoundedVolumeHierarchy4__dealloc__(FU8BoundedVolumeHierarchy4 *self)
{
    if (self->weakreflist)
    {
...
static PyObject *
FU8BoundedVolumeHierarchy4__repr__(FU8BoundedVolumeHierarchy4 *self)
{
...
static PyObject *
FU8BoundedVolumeHierarchy4_nodes(FU8BoundedVolumeHierarchy4 *self, void *closure)
{
    PyObject *bbox_cls = 0;
    PyObject *result = 0;
    ...
    {
        FU8BoundedVolumeHierarchy4Node *node = &self->nodes[n];
        ...
        for (int c = 0; c < 4; c++)
        {
            ...
            if (child_val & FU8BoundedVolumeHierarchy4_LEAF_MASK)
            {
                // leaf node - just emit the item index
                uint8_t item_idx = child_val & ~FU8BoundedVolumeHierarchy4_LEAF_MASK;
...
struct FU8BoundedVolumeHierarchy4RaycastIterator
{
    PyObject_HEAD
    FU8BoundedVolumeHierarchy4 *bvh;
    float eye[3];
    float inv_dir[3];
...
static void
FU8BoundedVolumeHierarchy4RaycastIterator__dealloc__(FU8BoundedVolumeHierarchy4RaycastIterator *self)
{
    Py_XDECREF(self->bvh);
    if (self->stack)
    {
...
static inline void
FU8BoundedVolumeHierarchy4_ray_test_node(
    const float *eye,
    const float *inv_dir,
    const FU8BoundedVolumeHierarchy4Node *node,
    uint8_t *hits,
    int *hit_count
)
{
...
static PyObject *
FU8BoundedVolumeHierarchy4RaycastIterator__next__(FU8BoundedVolumeHierarchy4RaycastIterator *self)
{
    FU8BoundedVolumeHierarchy4 *bvh = self->bvh;
    ...
    while (true)
    {
        ...
        FU8BoundedVolumeHierarchy4Node *node = &bvh->nodes[node_idx];

        // test all children of this node
        FU8BoundedVolumeHierarchy4_ray_test_node(self->eye, self->inv_dir, node, self->hits, &self->hit_count);
        self->hit_index = 0;
    }
}

static PyObject *
FU8BoundedVolumeHierarchy4RaycastIterator__iter__(FU8BoundedVolumeHierarchy4RaycastIterator *self)
{
    Py_INCREF(self);
    return (PyObject *)self;
}

static PyType_Slot FU8BoundedVolumeHierarchy4RaycastIterator_PyType_Slots[] = {
    {Py_tp_dealloc, (void *)FU8BoundedVolumeHierarchy4RaycastIterator__dealloc__},
    {Py_tp_iter, (void *)FU8BoundedVolumeHierarchy4RaycastIterator__iter__},
    {Py_tp_iternext, (void *)FU8BoundedVolumeHierarchy4RaycastIterator__next__},
    {0, 0},
};

static PyType_Spec FU8BoundedVolumeHierarchy4RaycastIterator_PyTypeSpec = {
    "egeometry.FU8BoundedVolumeHierarchy4RaycastIterator",
    sizeof(FU8BoundedVolumeHierarchy4RaycastIterator),
    0,
    Py_TPFLAGS_DEFAULT,
    FU8BoundedVolumeHierarchy4RaycastIterator_PyType_Slots
};

static PyTypeObject *FU8BoundedVolumeHierarchy4RaycastIterator_PyTypeObject = 0;

static PyObject *
FU8BoundedVolumeHierarchy4_raycast(FU8BoundedVolumeHierarchy4 *self, PyObject *const *args, Py_ssize_t nargs)
{
    if (nargs != 2)
    {
    ...
    // create iterator
    if (!FU8BoundedVolumeHierarchy4RaycastIterator_PyTypeObject)
    {
        FU8BoundedVolumeHierarchy4RaycastIterator_PyTypeObject = (PyTypeObject *)PyType_FromSpec(
            &FU8BoundedVolumeHierarchy4RaycastIterator_PyTypeSpec);
        if (!FU8BoundedVolumeHierarchy4RaycastIterator_PyTypeObject){ return 0; }
    }

    FU8BoundedVolumeHierarchy4RaycastIterator *iter = PyObject_New(
        FU8BoundedVolumeHierarchy4RaycastIterator, FU8BoundedVolumeHierarchy4RaycastIterator_PyTypeObject);
    if (!iter){ return 0; }
    ...
    {
        FU8BoundedVolumeHierarchy4_ray_test_node(iter->eye, iter->inv_dir, &self->nodes[0], iter->hits, &iter->hit_count);
    }
    else
    {
        iter->hit_count = 0;
...
static PyMethodDef FU8BoundedVolumeHierarchy4_PyMethodDef[] = {
    {"raycast", (PyCFunction)FU8BoundedVolumeHierarchy4_raycast, METH_FASTCALL, 0},
    {0, 0, 0, 0}
};

static PyMemberDef FU8BoundedVolumeHierarchy4_PyMemberDef[] = {
    {"__weaklistoffset__", T_PYSSIZET, offsetof(FU8BoundedVolumeHierarchy4, weakreflist), READONLY},
    {0, 0, 0, 0, 0},
};

static PyGetSetDef FU8BoundedVolumeHierarchy4_PyGetSetDef[] = {
    {"nodes", (getter)FU8BoundedVolumeHierarchy4_nodes, 0, 0, 0},
    {0, 0, 0, 0, 0}
};

static PyType_Slot FU8BoundedVolumeHierarchy4_PyType_Slots [] = {
    {Py_tp_new, (void*)FU8BoundedVolumeHierarchy4__new__},
    {Py_tp_dealloc, (void*)FU8BoundedVolumeHierarchy4__dealloc__},
    {Py_tp_repr, (void*)FU8BoundedVolumeHierarchy4__repr__},
    {Py_tp_members, (void*)FU8BoundedVolumeHierarchy4_PyMemberDef},
    {Py_tp_getset, (void*)FU8BoundedVolumeHierarchy4_PyGetSetDef},
    {Py_tp_methods, (void*)FU8BoundedVolumeHierarchy4_PyMethodDef},
    {0, 0},
};

static PyType_Spec FU8BoundedVolumeHierarchy4_PyTypeSpec = {
    "egeometry.FU8BoundedVolumeHierarchy4",
    sizeof(FU8BoundedVolumeHierarchy4),
    0,
    Py_TPFLAGS_DEFAULT,
    FU8BoundedVolumeHierarchy4_PyType_Slots
};

static PyTypeObject *
define_FU8BoundedVolumeHierarchy4_type(PyObject *module)
{
    PyTypeObject *type = (PyTypeObject *)PyType_FromModuleAndSpec(
        module,
        &FU8BoundedVolumeHierarchy4_PyTypeSpec,
        0
    );
    if (!type){ return 0; }
    // Note:
    ...
    if (PyModule_AddObject(module, "FU8BoundedVolumeHierarchy4", (PyObject *)type) < 0)
    {
        Py_DECREF(type);
        return 0;
    }

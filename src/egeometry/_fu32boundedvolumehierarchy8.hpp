...
#ifndef EGEOMETRY_FU32BOUNDEDVOLUMEHIERARCHY8_HPP
#define EGEOMETRY_FU32BOUNDEDVOLUMEHIERARCHY8_HPP
...
#include "_fu32boundedvolumehierarchy8type.hpp"
#include "_modulestate.hpp"

static const uint32_t FU32BoundedVolumeHierarchy8_LEAF_MASK (uint32_t(1) << (sizeof(uint32_t) * 8 - 1));

struct FU32BoundedVolumeHierarchy8Items
{
    size_t count;
    float *min_x;
    float *min_y;
...
static FU32BoundedVolumeHierarchy8Items
FU32BoundedVolumeHierarchy8_alloc_items_from_py_sequence_bounding_box_3d(
    PyObject *py_items
)
{
    ...
    FU32BoundedVolumeHierarchy8Items items;
    items.count = 0;
    items.min_x = 0;
...
static void
FU32BoundedVolumeHierarchy8_free_items(FU32BoundedVolumeHierarchy8Items items)
{
    if (items.min_x == 0)
    {
        free(items.min_x);
...
static void
FU32BoundedVolumeHierarchy8_alloc_nodes(
    FU32BoundedVolumeHierarchy8Items items,
...
)
{
    ...
    if (items.count == 0)
    {
        *nodes = (FU32BoundedVolumeHierarchy8Node *)malloc(1);
        if (!nodes)
        {
            PyErr_NoMemory();
            return;
    ...
    size_t max_item_count = (size_t)(uint32_t)(~FU32BoundedVolumeHierarchy8_LEAF_MASK) + 1;
    ...
    float *center_y = 0;
    float *center_z = 0;
    FU32BoundedVolumeHierarchy8Node *node_array = 0;
    ...
    node_array = (FU32BoundedVolumeHierarchy8Node *)malloc(sizeof(FU32BoundedVolumeHierarchy8Node) * node_array_count);
    if (!node_array){ PyErr_NoMemory(); goto error; }
    memset(node_array, 0, sizeof(FU32BoundedVolumeHierarchy8Node) * node_array_count);
...

    {
        ...
        auto fill_child = [&](size_t node_idx, int c, size_t child_start, size_t child_end) -> bool
        {
            ...
            else if (child_count_items == 1)
            {
                ...
                node_array[node_idx].child[c] = item_idx | FU32BoundedVolumeHierarchy8_LEAF_MASK;
                return true;
            }
            else
            {
        ...
        {
            FU32BoundedVolumeHierarchy8Node *shrunk = (FU32BoundedVolumeHierarchy8Node *)realloc(node_array, sizeof(FU32BoundedVolumeHierarchy8Node) * next_node);
...
static void
FU32BoundedVolumeHierarchy8_free_nodes(FU32BoundedVolumeHierarchy8Node *nodes)
{
    if (nodes)
    {
        free(nodes);
...
static PyObject *
FU32BoundedVolumeHierarchy8__new__(PyTypeObject *cls, PyObject *args, PyObject *kwds)
{
    FU32BoundedVolumeHierarchy8 *self = 0;
    FU32BoundedVolumeHierarchy8Node *nodes = 0;

    static const char *kwp[] = {"items", 0};
    PyObject *py_items = 0;
    if (!PyArg_ParseTupleAndKeywords(args, kwds, "O", (char **)kwp, &py_items)){ return 0; }

    FU32BoundedVolumeHierarchy8Items items = FU32BoundedVolumeHierarchy8_alloc_items_from_py_sequence_bounding_box_3d(py_items);
    if (items.min_x == 0){ goto error; }

    size_t node_count;
    FU32BoundedVolumeHierarchy8_alloc_nodes(items, &nodes, &node_count);
    if (!nodes && items.count > 0){ goto error; }

    FU32BoundedVolumeHierarchy8_free_items(items);

    self = (FU32BoundedVolumeHierarchy8*)cls->tp_alloc(cls, 0);
    if (!self)
    {
        goto error;
    }
...
error:
    FU32BoundedVolumeHierarchy8_free_nodes(nodes);
    FU32BoundedVolumeHierarchy8_free_items(items);
    return 0;
}

static void
FU32BoundedVolumeHierarchy8__dealloc__(FU32BoundedVolumeHierarchy8 *self)
{
    if (self->weakreflist)
    {
...
static PyObject *
FU32BoundedVolumeHierarchy8__repr__(FU32BoundedVolumeHierarchy8 *self)
{
...
static PyObject *
FU32BoundedVolumeHierarchy8_nodes(FU32BoundedVolumeHierarchy8 *self, void *closure)
{
    PyObject *bbox_cls = 0;
    PyObject *result = 0;
    ...
    {
        FU32BoundedVolumeHierarchy8Node *node = &self->nodes[n];
        ...
        for (int c = 0; c < 8; c++)
        {
            ...
            if (child_val & FU32BoundedVolumeHierarchy8_LEAF_MASK)
            {
                // leaf node - just emit the item index
                uint32_t item_idx = child_val & ~FU32BoundedVolumeHierarchy8_LEAF_MASK;
...
struct FU32BoundedVolumeHierarchy8RaycastIterator
{
    PyObject_HEAD
    FU32BoundedVolumeHierarchy8 *bvh;
    float eye[3];
    float inv_dir[3];
...
static void
FU32BoundedVolumeHierarchy8RaycastIterator__dealloc__(FU32BoundedVolumeHierarchy8RaycastIterator *self)
{
    Py_XDECREF(self->bvh);
    if (self->stack)
    {
...
static inline void
FU32BoundedVolumeHierarchy8_ray_test_node(
    const float *eye,
    const float *inv_dir,
    const FU32BoundedVolumeHierarchy8Node *node,
    uint32_t *hits,
    int *hit_count
)
{
...
static PyObject *
FU32BoundedVolumeHierarchy8RaycastIterator__next__(FU32BoundedVolumeHierarchy8RaycastIterator *self)
{
    FU32BoundedVolumeHierarchy8 *bvh = self->bvh;
    ...
    while (true)
    {
        ...
        FU32BoundedVolumeHierarchy8Node *node = &bvh->nodes[node_idx];

        // test all children of this node
        FU32BoundedVolumeHierarchy8_ray_test_node(self->eye, self->inv_dir, node, self->hits, &self->hit_count);
        self->hit_index = 0;
    }
}

static PyObject *
FU32BoundedVolumeHierarchy8RaycastIterator__iter__(FU32BoundedVolumeHierarchy8RaycastIterator *self)
{
    Py_INCREF(self);
    return (PyObject *)self;
}

static PyType_Slot FU32BoundedVolumeHierarchy8RaycastIterator_PyType_Slots[] = {
    {Py_tp_dealloc, (void *)FU32BoundedVolumeHierarchy8RaycastIterator__dealloc__},
    {Py_tp_iter, (void *)FU32BoundedVolumeHierarchy8RaycastIterator__iter__},
    {Py_tp_iternext, (void *)FU32BoundedVolumeHierarchy8RaycastIterator__next__},
    {0, 0},
};

static PyType_Spec FU32BoundedVolumeHierarchy8RaycastIterator_PyTypeSpec = {
    "egeometry.FU32BoundedVolumeHierarchy8RaycastIterator",
    sizeof(FU32BoundedVolumeHierarchy8RaycastIterator),
    0,
    Py_TPFLAGS_DEFAULT,
    FU32BoundedVolumeHierarchy8RaycastIterator_PyType_Slots
};

static PyTypeObject *FU32BoundedVolumeHierarchy8RaycastIterator_PyTypeObject = 0;

static PyObject *
FU32BoundedVolumeHierarchy8_raycast(FU32BoundedVolumeHierarchy8 *self, PyObject *const *args, Py_ssize_t nargs)
{
    if (nargs != 2)
    {
    ...
    // create iterator
    if (!FU32BoundedVolumeHierarchy8RaycastIterator_PyTypeObject)
    {
        FU32BoundedVolumeHierarchy8RaycastIterator_PyTypeObject = (PyTypeObject *)PyType_FromSpec(
            &FU32BoundedVolumeHierarchy8RaycastIterator_PyTypeSpec);
        if (!FU32BoundedVolumeHierarchy8RaycastIterator_PyTypeObject){ return 0; }
    }

    FU32BoundedVolumeHierarchy8RaycastIterator *iter = PyObject_New(
        FU32BoundedVolumeHierarchy8RaycastIterator, FU32BoundedVolumeHierarchy8RaycastIterator_PyTypeObject);
    if (!iter){ return 0; }
    ...
    {
        FU32BoundedVolumeHierarchy8_ray_test_node(iter->eye, iter->inv_dir, &self->nodes[0], iter->hits, &iter->hit_count);
    }
    else
    {
        iter->hit_count = 0;
...
static PyMethodDef FU32BoundedVolumeHierarchy8_PyMethodDef[] = {
    {"raycast", (PyCFunction)FU32BoundedVolumeHierarchy8_raycast, METH_FASTCALL, 0},
    {0, 0, 0, 0}
};

static PyMemberDef FU32BoundedVolumeHierarchy8_PyMemberDef[] = {
    {"__weaklistoffset__", T_PYSSIZET, offsetof(FU32BoundedVolumeHierarchy8, weakreflist), READONLY},
    {0, 0, 0, 0, 0},
};

static PyGetSetDef FU32BoundedVolumeHierarchy8_PyGetSetDef[] = {
    {"nodes", (getter)FU32BoundedVolumeHierarchy8_nodes, 0, 0, 0},
    {0, 0, 0, 0, 0}
};

static PyType_Slot FU32BoundedVolumeHierarchy8_PyType_Slots [] = {
    {Py_tp_new, (void*)FU32BoundedVolumeHierarchy8__new__},
    {Py_tp_dealloc, (void*)FU32BoundedVolumeHierarchy8__dealloc__},
    {Py_tp_repr, (void*)FU32BoundedVolumeHierarchy8__repr__},
    {Py_tp_members, (void*)FU32BoundedVolumeHierarchy8_PyMemberDef},
    {Py_tp_getset, (void*)FU32BoundedVolumeHierarchy8_PyGetSetDef},
    {Py_tp_methods, (void*)FU32BoundedVolumeHierarchy8_PyMethodDef},
    {0, 0},
};

static PyType_Spec FU32BoundedVolumeHierarchy8_PyTypeSpec = {
    "egeometry.FU32BoundedVolumeHierarchy8",
    sizeof(FU32BoundedVolumeHierarchy8),
    0,
    Py_TPFLAGS_DEFAULT,
    FU32BoundedVolumeHierarchy8_PyType_Slots
};

static PyTypeObject *
define_FU32BoundedVolumeHierarchy8_type(PyObject *module)
{
    PyTypeObject *type = (PyTypeObject *)PyType_FromModuleAndSpec(
        module,
        &FU32BoundedVolumeHierarchy8_PyTypeSpec,
        0
    );
    if (!type){ return 0; }
    // Note:
    ...
    if (PyModule_AddObject(module, "FU32BoundedVolumeHierarchy8", (PyObject *)type) < 0)
    {
        Py_DECREF(type);
        return 0;
    }

...
#ifndef EGEOMETRY_DUBOUNDEDVOLUMEHIERARCHY4_HPP
#define EGEOMETRY_DUBOUNDEDVOLUMEHIERARCHY4_HPP
...
#include "_duboundedvolumehierarchy4type.hpp"
#include "_modulestate.hpp"

static const unsigned int DUBoundedVolumeHierarchy4_LEAF_MASK (((unsigned int)1) << (sizeof(unsigned int) * 8 - 1));

struct DUBoundedVolumeHierarchy4Items
{
    size_t count;
    double *min_x;
    double *min_y;
...
static DUBoundedVolumeHierarchy4Items
DUBoundedVolumeHierarchy4_alloc_items_from_py_sequence_bounding_box_3d(
    PyObject *py_items
)
{
    ...
    DUBoundedVolumeHierarchy4Items items;
    items.count = 0;
    items.min_x = 0;
...
static void
DUBoundedVolumeHierarchy4_free_items(DUBoundedVolumeHierarchy4Items items)
{
    if (items.min_x == 0)
    {
        free(items.min_x);
...
static void
DUBoundedVolumeHierarchy4_alloc_nodes(
    DUBoundedVolumeHierarchy4Items items,
...
)
{
    ...
    if (items.count == 0)
    {
        *nodes = (DUBoundedVolumeHierarchy4Node *)malloc(1);
        if (!nodes)
        {
            PyErr_NoMemory();
            return;
    ...
    size_t max_item_count = (size_t)(unsigned int)(~DUBoundedVolumeHierarchy4_LEAF_MASK) + 1;
    ...
    double *center_y = 0;
    double *center_z = 0;
    DUBoundedVolumeHierarchy4Node *node_array = 0;
    ...
    node_array = (DUBoundedVolumeHierarchy4Node *)malloc(sizeof(DUBoundedVolumeHierarchy4Node) * node_array_count);
    if (!node_array){ PyErr_NoMemory(); goto error; }
    memset(node_array, 0, sizeof(DUBoundedVolumeHierarchy4Node) * node_array_count);
...

    {
        ...
        auto fill_child = [&](size_t node_idx, int c, size_t child_start, size_t child_end) -> bool
        {
            ...
            else if (child_count_items == 1)
            {
                ...
                node_array[node_idx].child[c] = item_idx | DUBoundedVolumeHierarchy4_LEAF_MASK;
                return true;
            }
            else
            {
        ...
        {
            DUBoundedVolumeHierarchy4Node *shrunk = (DUBoundedVolumeHierarchy4Node *)realloc(node_array, sizeof(DUBoundedVolumeHierarchy4Node) * next_node);
...
static void
DUBoundedVolumeHierarchy4_free_nodes(DUBoundedVolumeHierarchy4Node *nodes)
{
    if (nodes)
    {
        free(nodes);
...
static PyObject *
DUBoundedVolumeHierarchy4__new__(PyTypeObject *cls, PyObject *args, PyObject *kwds)
{
    DUBoundedVolumeHierarchy4 *self = 0;
    DUBoundedVolumeHierarchy4Node *nodes = 0;

    static const char *kwp[] = {"items", 0};
    PyObject *py_items = 0;
    if (!PyArg_ParseTupleAndKeywords(args, kwds, "O", (char **)kwp, &py_items)){ return 0; }

    DUBoundedVolumeHierarchy4Items items = DUBoundedVolumeHierarchy4_alloc_items_from_py_sequence_bounding_box_3d(py_items);
    if (items.min_x == 0){ goto error; }

    size_t node_count;
    DUBoundedVolumeHierarchy4_alloc_nodes(items, &nodes, &node_count);
    if (!nodes && items.count > 0){ goto error; }

    DUBoundedVolumeHierarchy4_free_items(items);

    self = (DUBoundedVolumeHierarchy4*)cls->tp_alloc(cls, 0);
    if (!self)
    {
        goto error;
    }
...
error:
    DUBoundedVolumeHierarchy4_free_nodes(nodes);
    DUBoundedVolumeHierarchy4_free_items(items);
    return 0;
}

static void
DUBoundedVolumeHierarchy4__dealloc__(DUBoundedVolumeHierarchy4 *self)
{
    if (self->weakreflist)
    {
...
static PyObject *
DUBoundedVolumeHierarchy4__repr__(DUBoundedVolumeHierarchy4 *self)
{
...
static PyObject *
DUBoundedVolumeHierarchy4_nodes(DUBoundedVolumeHierarchy4 *self, void *closure)
{
    PyObject *bbox_cls = 0;
    PyObject *result = 0;
    ...
    {
        DUBoundedVolumeHierarchy4Node *node = &self->nodes[n];
        ...
        for (int c = 0; c < 4; c++)
        {
            ...
            if (child_val & DUBoundedVolumeHierarchy4_LEAF_MASK)
            {
                // leaf node - just emit the item index
                unsigned int item_idx = child_val & ~DUBoundedVolumeHierarchy4_LEAF_MASK;
...
struct DUBoundedVolumeHierarchy4RaycastIterator
{
    PyObject_HEAD
    DUBoundedVolumeHierarchy4 *bvh;
    double eye[3];
    double inv_dir[3];
...
static void
DUBoundedVolumeHierarchy4RaycastIterator__dealloc__(DUBoundedVolumeHierarchy4RaycastIterator *self)
{
    Py_XDECREF(self->bvh);
    if (self->stack)
    {
...
static inline void
DUBoundedVolumeHierarchy4_ray_test_node(
    const double *eye,
    const double *inv_dir,
    const DUBoundedVolumeHierarchy4Node *node,
    unsigned int *hits,
    int *hit_count
)
{
...
static PyObject *
DUBoundedVolumeHierarchy4RaycastIterator__next__(DUBoundedVolumeHierarchy4RaycastIterator *self)
{
    DUBoundedVolumeHierarchy4 *bvh = self->bvh;
    ...
    while (true)
    {
        ...
        DUBoundedVolumeHierarchy4Node *node = &bvh->nodes[node_idx];

        // test all children of this node
        DUBoundedVolumeHierarchy4_ray_test_node(self->eye, self->inv_dir, node, self->hits, &self->hit_count);
        self->hit_index = 0;
    }
}

static PyObject *
DUBoundedVolumeHierarchy4RaycastIterator__iter__(DUBoundedVolumeHierarchy4RaycastIterator *self)
{
    Py_INCREF(self);
    return (PyObject *)self;
}

static PyType_Slot DUBoundedVolumeHierarchy4RaycastIterator_PyType_Slots[] = {
    {Py_tp_dealloc, (void *)DUBoundedVolumeHierarchy4RaycastIterator__dealloc__},
    {Py_tp_iter, (void *)DUBoundedVolumeHierarchy4RaycastIterator__iter__},
    {Py_tp_iternext, (void *)DUBoundedVolumeHierarchy4RaycastIterator__next__},
    {0, 0},
};

static PyType_Spec DUBoundedVolumeHierarchy4RaycastIterator_PyTypeSpec = {
    "egeometry.DUBoundedVolumeHierarchy4RaycastIterator",
    sizeof(DUBoundedVolumeHierarchy4RaycastIterator),
    0,
    Py_TPFLAGS_DEFAULT,
    DUBoundedVolumeHierarchy4RaycastIterator_PyType_Slots
};

static PyTypeObject *DUBoundedVolumeHierarchy4RaycastIterator_PyTypeObject = 0;

static PyObject *
DUBoundedVolumeHierarchy4_raycast(DUBoundedVolumeHierarchy4 *self, PyObject *const *args, Py_ssize_t nargs)
{
    if (nargs != 2)
    {
    ...
    // create iterator
    if (!DUBoundedVolumeHierarchy4RaycastIterator_PyTypeObject)
    {
        DUBoundedVolumeHierarchy4RaycastIterator_PyTypeObject = (PyTypeObject *)PyType_FromSpec(
            &DUBoundedVolumeHierarchy4RaycastIterator_PyTypeSpec);
        if (!DUBoundedVolumeHierarchy4RaycastIterator_PyTypeObject){ return 0; }
    }

    DUBoundedVolumeHierarchy4RaycastIterator *iter = PyObject_New(
        DUBoundedVolumeHierarchy4RaycastIterator, DUBoundedVolumeHierarchy4RaycastIterator_PyTypeObject);
    if (!iter){ return 0; }
    ...
    {
        DUBoundedVolumeHierarchy4_ray_test_node(iter->eye, iter->inv_dir, &self->nodes[0], iter->hits, &iter->hit_count);
    }
    else
    {
        iter->hit_count = 0;
...
static PyMethodDef DUBoundedVolumeHierarchy4_PyMethodDef[] = {
    {"raycast", (PyCFunction)DUBoundedVolumeHierarchy4_raycast, METH_FASTCALL, 0},
    {0, 0, 0, 0}
};

static PyMemberDef DUBoundedVolumeHierarchy4_PyMemberDef[] = {
    {"__weaklistoffset__", T_PYSSIZET, offsetof(DUBoundedVolumeHierarchy4, weakreflist), READONLY},
    {0, 0, 0, 0, 0},
};

static PyGetSetDef DUBoundedVolumeHierarchy4_PyGetSetDef[] = {
    {"nodes", (getter)DUBoundedVolumeHierarchy4_nodes, 0, 0, 0},
    {0, 0, 0, 0, 0}
};

static PyType_Slot DUBoundedVolumeHierarchy4_PyType_Slots [] = {
    {Py_tp_new, (void*)DUBoundedVolumeHierarchy4__new__},
    {Py_tp_dealloc, (void*)DUBoundedVolumeHierarchy4__dealloc__},
    {Py_tp_repr, (void*)DUBoundedVolumeHierarchy4__repr__},
    {Py_tp_members, (void*)DUBoundedVolumeHierarchy4_PyMemberDef},
    {Py_tp_getset, (void*)DUBoundedVolumeHierarchy4_PyGetSetDef},
    {Py_tp_methods, (void*)DUBoundedVolumeHierarchy4_PyMethodDef},
    {0, 0},
};

static PyType_Spec DUBoundedVolumeHierarchy4_PyTypeSpec = {
    "egeometry.DUBoundedVolumeHierarchy4",
    sizeof(DUBoundedVolumeHierarchy4),
    0,
    Py_TPFLAGS_DEFAULT,
    DUBoundedVolumeHierarchy4_PyType_Slots
};

static PyTypeObject *
define_DUBoundedVolumeHierarchy4_type(PyObject *module)
{
    PyTypeObject *type = (PyTypeObject *)PyType_FromModuleAndSpec(
        module,
        &DUBoundedVolumeHierarchy4_PyTypeSpec,
        0
    );
    if (!type){ return 0; }
    // Note:
    ...
    if (PyModule_AddObject(module, "DUBoundedVolumeHierarchy4", (PyObject *)type) < 0)
    {
        Py_DECREF(type);
        return 0;
    }
